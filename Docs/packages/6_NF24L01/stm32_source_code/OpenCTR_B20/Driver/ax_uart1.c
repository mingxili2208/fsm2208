		...
		else    //==接收数据
		{
			...
			else    //判断最后1位
			{
				...
				{	
					...
					{
						//舵机角度控制帧
						if(uart1_rx_buf[3] == ID_URX_ANGLE)
						{
							ax_joint_angle[0] = (int16_t)((uart1_rx_buf[4]<<8) | uart1_rx_buf[5]);
							ax_joint_angle[1] = (int16_t)((uart1_rx_buf[6]<<8) | uart1_rx_buf[7]);
						}	
					}	
				}
			}
