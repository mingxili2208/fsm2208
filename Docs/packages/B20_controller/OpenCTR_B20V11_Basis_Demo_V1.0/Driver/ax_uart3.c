...
static uint8_t uart3_flag_rx_ok = 0; //接收成功标志
		...
		else    //==接收数据
		{
			...
			else    //判断最后1位
			{
				...
				{	
					...
					//printf("ID = %d \r\n",uart3_rx_buf[4],uart3_rx_buf[4],uart3_rx_buf[4]);
				}	
			}
		}
...
/**
  ...
  */
uint8_t AX_UART3_GetData(uint8_t *pbuf)
{
	uint8_t cnt,i;
	
	if(uart3_flag_rx_ok != 0)
	{
		cnt = uart3_rx_buf[2]-4;
		
		for(i=0; i<cnt; i++)
		{
			*(pbuf+i) = uart3_rx_buf[3+i];
		}
