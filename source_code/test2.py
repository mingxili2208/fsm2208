...
{'position': [3.01, 0.004, 0.573], 'orientation': [np.radians(0), np.radians(0), np.radians(0)]}, #11
{'position': [3.01, 0.004, 0.573], 'orientation': [np.radians(-90), np.radians(30), np.radians(0)]}, #11'
{'position': [3.01, 0.004, 0.573], 'orientation': [np.radians(30), np.radians(0), np.radians(-20)]}, #11''
]
...
# 1. 计算法线 (对于某些配准算法是必需的)
pcd_A.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
pcd_B.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))

# 2. 创建点云配准对象 (这里使用 ICP 算法为例)
# 设置criteria参数，使用默认的RANSAC终止条件
criteria = o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=30)
icp = o3d.pipelines.registration.registration_icp(
    pcd_A, pcd_B, max_correspondence_distance=0.01,  # 直接使用原始点云
...
# 将点云A转换到点云B的坐标系
pcd_A_transformed = pcd_A.transform(transformation)

# 创建可视化窗口并显示配准后的点云
o3d.visualization.draw_geometries([pcd_A_transformed, pcd_B])