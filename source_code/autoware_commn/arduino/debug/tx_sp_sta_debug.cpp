...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (commaIndex != -1) {
      float steering_tire_angle = input.substring(0, commaIndex).toFloat();
      float speed = input.substring(commaIndex + 1).toFloat();

      //steering_tire_angle，speed
      set_txbuff(steering_tire_angle,speed);
      
      bool ok = radio.write(&tx_buf, sizeof(tx_buf));
      //////////////////////////////////////////////////////////////////for test
      if (ok) {
        Serial.println("radio send succeed ");
        Serial.print(steering_tire_angle);
