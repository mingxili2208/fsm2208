...
class VehicleFolloweHandler(Node):
    ...
    def __init__(self,update_frequency=100):
        super().__init__('vehicle_follow_control_ndoe')
        self.agent_role_name = os.environ["AGENT_ROLE_NAME"].replace("-", "")
        self.update_frequency = update_frequency
        self.ego_vehicle = None
        self.world = CarlaDataProvider.get_world()
        self.ego_vehicle = BridgeHelpers.get_agent_actor(self.world, self.agent_role_name)
        self.topic_name=f"/real_world/{VEHICLE_NAME.replace('-', '_')}/transformed"
        if self.ego_vehicle is None:
            self.get_logger().error(f"[VehicleFollower] Can't find Ego Vehicle named {self.agent_role_name}!! Make sure it is spawned!")
            raise RuntimeError(f"Can't find Ego Vehicle named {self.agent_role_name}!! Make sure it is spawned!")
    ...
    def on_real_vehicle_transform_callback(self, data: PoseStamped):
            """
...
if __name__ == "__main__":
    if os.environ["CONTROL_MODE"] != "pygame" and os.environ["RUNNING_MODE"] == "record":
        ...
        ego_follow_handler = VehicleFolloweHandler()
