...
def main():
    ...
    # 确保我们有足够的索引
    num_vehicles = min(args.num_vehicles, len(spawn_indices))
    
    # 创建车辆蓝图
    blueprint_library = world.get_blueprint_library()
    # 筛选所有车辆，但排除奥迪车型和自行车
    vehicle_blueprints = [bp for bp in blueprint_library.filter('vehicle.*') 
                         if 'audi' not in bp.id.lower() and 'bicycle' not in bp.id.lower()]
    ...
    try:
        ...
        for i in range(num_vehicles):
            ...
            # 在指定的出生点生成车辆
            spawn_idx = spawn_indices[i % len(spawn_indices)]
            spawn_point = spawn_points[spawn_idx]
            
            # 修改spawn_point的高度为0.2（不再使用默认的4）
            ...
            while vehicle is None and spawn_attempts < 5:
                ...
                # 同样修改随机选择的出生点高度
                random_spawn.location.z = 0.2
                print(f"  尝试备用位置 {spawn_attempts}...")
                vehicle = world.try_spawn_actor(bp, random_spawn)
            ...
            if vehicle is not None:
                ...
                # 设置目标点
                target_idx = target_indices[i % len(target_indices)]
