...
try:
    ...
    from geometry_msgs.msg import PoseStamped  # 添加PoseStamped导入
    ...
    import tf_transformations  # 导入quaternion转换函数
...
def quat2euler(quat):
    ...
    try:
        # quat格式为[w, x, y, z]
        return tf_transformations.euler_from_quaternion(quat)
    except NameError:
        # 如果tf_transformations未导入，返回默认值
        logging.warning("tf_transformations未导入，无法计算欧拉角")
        return (0.0, 0.0, 0.0)


class EngagePublisher(Node):
    """
    用于向Autoware发送engage命令和目标点的ROS2节点
    """
    def __init__(self):
    ...
    def publish_goal(self, goal_pos):
        ...
        try:
            ...
            yaw = 0.523576  # 默认值，与示例相同
        
        self.goal_publisher.publish(msg)
        
        self.get_logger().info(
            f"发布目标点: Frame:map, 位置({goal_pos[0]}, {goal_pos[1]}, {goal_pos[2]}), "
            f"方向({goal_pos[3]}, {goal_pos[4]}, {goal_pos[5]}, {goal_pos[6]}) = 角度: {yaw}"
        )
...
class CarlaManager:
    ...
    def send_goal_and_engage_autoware(self):
        """向Autoware发送目标点和engage指令"""
        if not self.args.engage_autoware or not ROS2_AVAILABLE:
            if not ROS2_AVAILABLE:
                logging.warning("ROS2未加载，无法发送goal和engage指令")
            return False
        
        try:
            logging.info('尝试向Autoware发送目标点和engage指令...')
            ...
            # 创建发布者
            publisher = EngagePublisher()
            
            # 等待/clock话题
            clock_received = publisher.wait_for_clock(5.0)
            
            # 预定义的目标点坐标 [x, y, z, qx, qy, qz, qw]
            goal_position = [19.1288, -43.0747, 0.0, 0.0, 0.0, 0.258808, 0.965929]
            
            # 第1步：发送目标点
            logging.info('第1步：发送目标点...')
            publisher.publish_goal(goal_position)
            
            # 确保目标点消息被发送
            for _ in range(5):  # 自旋几次以确保消息发送
                rclpy.spin_once(publisher, timeout_sec=0.1)
            
            # 等待3秒
            logging.info('等待3秒，让Autoware处理目标点...')
            time.sleep(3.0)
            
            # 第2步：发送engage命令
            logging.info('第2步：发送engage指令...')
            publisher.publish_engage(True)
            
            # 确保engage消息被发送
            for _ in range(5):  # 自旋几次以确保消息发送
                rclpy.spin_once(publisher, timeout_sec=0.1)
            
            logging.info('成功向Autoware发送目标点和engage指令')
            ...
            logging.error(f'发送目标点和engage指令时出错: {e}')
            # 打印详细的堆栈跟踪
            import traceback
...
def main():
    ...
    try:
        ...
        # 修改流程：首先发送目标点和engage指令
        if args.engage_autoware:
            logging.info("向Autoware发送目标点和engage指令...")
            success = carla_manager.send_goal_and_engage_autoware()
            if success:
                # 等待2秒让Autoware处理engage命令
                logging.info("等待Autoware处理engage指令 (2秒)...")
                time.sleep(2)
            else:
                logging.warning("向Autoware发送指令失败，将继续生成NPC车辆")
