...
class IntegratedControlPublisher(Node):
    ...
    def __init__(self):
        ...
        # 设置日志文件路径
        self.log_file_path = 'serial_output.log'

        # 管理打开的终端窗口
        self.log_terminal_processes = []
    ...
    def verify_checksum(self, data):
        ...
        try:
            ...
            return False
        calculated_checksum = self.calculate_checksum(steering_tire_angle, speed)
    ...
    def listener_callback(self, msg):
        ...
        if (self.pre_steering_tire_angle is None or
            ...
            if (self.last_sent_commands['speed'] != speed or
                ...
                # 将发送的消息写入日志文件
                self.write_to_log_file(f"Sent: {hex_msg}")
                ...
                while not self.data_queue.empty():
                    self.logger.info("not empty data queue")
    ...
    def read_from_serial(self):
        with open('serial_output.log', 'w') as log_file:
                ...
                try:
                    if self.ser.in_waiting > 0:
                        ...
                        if response_data:
                            ...
                            # 仅将接收到的数据写入日志文件，不打印到主终端
                            log_file.write(f"Received: {hex_data}\n")
                            log_file.flush()
                    else:
    ...
    def close_log_terminals(self):
        """
        关闭所有通过 open_log_terminal 打开的终端窗口。
        """
        for proc in self.log_terminal_processes:
            if proc.poll() is None:  # 进程仍在运行
                try:
                    proc.terminate()
                    proc.wait(timeout=1)
                    self.logger.info("已关闭一个日志终端窗口。")
                except subprocess.TimeoutExpired:
                    proc.kill()
                    self.logger.warning("无法正常终止日志终端，已强制关闭。")
        self.log_terminal_processes = []

    def open_log_terminal(self):
        """
        打开一个新的终端窗口，并在其中运行 `tail -f serial_output.log` 以实时显示串口数据。
        """
        try:
            if sys.platform.startswith('linux') or sys.platform.startswith('darwin'):
                # 使用 gnome-terminal 或 xterm
                if shutil.which('gnome-terminal'):
                    # gnome-terminal ---- bash -c 'tail -f serial_output.log; exit'
                    process = subprocess.Popen(['gnome-terminal', '--', 'bash', '-c', f'tail -f {self.log_file_path}; exit'])
                    self.log_terminal_processes.append(process)
                elif shutil.which('xterm'):
                    # xterm 不使用 '-hold' 以便进程可以被终止
                    process = subprocess.Popen(['xterm', '-e', f'tail -f {self.log_file_path}'])
                    self.log_terminal_processes.append(process)
                else:
                    self.logger.warning("没有找到 gnome-terminal 或 xterm，无法打开独立终端显示串口数据。")
            elif sys.platform.startswith('win'):
                # Windows 使用 PowerShell
                # PowerShell 会保持打开状态，需使用 `exit` 来关闭
                # Run 'Get-Content serial_output.log -Wait; exit'
                process = subprocess.Popen(['powershell', '-Command', f"Get-Content {self.log_file_path} -Wait; exit"])
                self.log_terminal_processes.append(process)
            else:
                self.logger.warning("不支持的操作系统，无法打开独立终端显示串口数据。")
        except Exception as e:
            self.logger.error(f"打开独立终端时发生错误: {e}")
            ...
            if self.mode == self.MODE_MANUAL:
                ...
                with self.cmd_lock:
                    ...
                    if 'w' in self.key_state:
                        ...
                        new_speed = self.speed  # 无按键时保持当前速度
    ...
    def send_serial_command(self, steering_angle_deg, speed):
        try:
            ...
            # 计算校验和
            checksum = self.calculate_checksum(steering_angle_deg, speed)
