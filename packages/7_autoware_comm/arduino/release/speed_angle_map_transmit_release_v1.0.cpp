...
this is in a high speed;

changing speed map max by change the param "max_mapped" of func "map_speed_sigmoid()"
...
// 速度映射函数，应用sigmoid函数
int map_speed_sigmoid(float speed, float min_speed = 0.20, float max_speed = 1.5, float min_mapped = 5, float max_mapped = 55) {
  // 设置速度阈值
  if (speed <= 0.05) {
    return 0;
  }
