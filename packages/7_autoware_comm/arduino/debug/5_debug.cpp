...
void setup() {
  ...
  interp.addPoint(0, 0);
  interp.addPoint(2, 8);
  interp.addPoint(5, 13);
  interp.addPoint(8, 18);
  interp.addPoint(12, 25);
  interp.addPoint(15, 50);
  interp.addPoint(20, 70);
  interp.addPoint(25, 95);
  interp.addPoint(30, 110);
 // interp.addPoint(40, 120);
...
//      control_command.speed = (float)speed;
    ...
    {//data receive but analysis error-----------
        sendStateMessage(data[0],control_command.steering_tire_angle, control_command.speed);
...
//   Serial.println();
// }
int map_speed_sigmoid(float speed, float min_speed = 0.05, float max_speed = 1.8, float min_mapped = 35, float max_mapped = 55) {
  // 设置速度阈值
  if (speed <= 0.05) {
    return 0;
  }
