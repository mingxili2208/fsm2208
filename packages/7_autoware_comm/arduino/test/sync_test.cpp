...
int map_speed_sigmoid(float speed, float min_speed = 0.20, float max_speed = 1.3, float min_mapped = 0, float max_mapped = 55);
void setup() {
...
void processSyncPacket() {
  ...
  if (data[0] == TIME_SYNC_HEADER) {
    ...
    sendSyncAck(pcTimestamp);
  }
}

// 发送时间同步ACK
void sendSyncAck(unsigned long pcTimestamp) {
  byte response[10]; 
...
void set_txbuff(byte mapped_steering, byte mapped_speed) {
    ...
    tx_buf[4] = 0x00;             // steering_angle_velocity, 当前模式未使用角速度控制，设为 0
