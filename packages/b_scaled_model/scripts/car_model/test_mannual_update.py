...
class VehicleSpawner(Node):
    ...
    def load_vehicle(self):
        ...
        self.spawn_point.location.x = -6.15
        self.spawn_point.location.y = 56.58  # ROS坐标系与CARLA坐标系的y轴相反
        self.spawn_point.location.z = 0.0048 + 1.0  # 微调Z高度，避免车辆嵌入地面

        # 四元数转欧拉角（CARLA 使用欧拉角来表示旋转）
        # orientation_q = msg.pose.pose.orientation
        # roll, pitch, yaw = quat2euler([orientation_q.w, orientation_q.x, orientation_q.y, orientation_q.z])
        self.spawn_point.rotation.roll = math.degrees(0)
        self.spawn_point.rotation.pitch = math.degrees(0)
        self.spawn_point.rotation.yaw = -math.degrees(0)  # ROS到CARLA的yaw角调整
        ...
        if VehicleSpawner.get_agent_actor(CarlaDataProvider.get_world(), self.agent_role_name) is None:
            ...
            # 生成车辆
            self.ego=CarlaDataProvider.request_new_actor(
        ...
        if self.ego is not None:
            ...
            pose = self.spawn_point
            pose.location.x = 10.15
            self.ego.set_transform(pose)
            print("Vehicle moved to new position.")
