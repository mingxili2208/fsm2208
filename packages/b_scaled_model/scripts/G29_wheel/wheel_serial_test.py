...
class IntegratedControlPublisher(Node):
    ...
    def init_wheel_device(self):
        ...
        try:
            ...
            if device_path:
                ...
                self.logger.info(f"成功初始化方向盘设备: {self.wheel_device.name}")
                
                # 尝试禁用力反馈
                if ecodes.FF_AUTOCENTER in self.wheel_device.capabilities().get(ecodes.EV_FF, []):
    ...
    def find_wheel_device(self):
        """查找方向盘设备路径"""
        self.logger.debug(f"查找方向盘设备: {self.WHEEL_DEVICE_NAME}")
        try:
            for path in evdev.list_devices():
                try:
                    device = evdev.InputDevice(path)
                    if self.WHEEL_DEVICE_NAME.lower() in device.name.lower():
                        self.logger.info(f"找到设备: {device.name} ({path})")
                        return path
                    else:
                        self.logger.debug(f"跳过设备: {device.name} ({path})")
                except Exception as e:
                    self.logger.debug(f"无法访问设备 {path}: {e}")
        except Exception as e:
    ...
    def wheel_event_listener(self):
        """监听方向盘设备事件的线程"""
        self.logger.debug("方向盘事件监听线程已启动")
                ...
                if event.type in EVENT_CODE_TO_KEY and event.code in EVENT_CODE_TO_KEY[event.type]:
                    ...
                    if event.type == ecodes.EV_ABS:
                        ...
                        if "func" in control:
                            ...
                            self.logger.debug(f"{control['name']}: 原始值={event.value}, 映射值={mapped_value}")
                    ...
                    # 对于按钮事件(按键)
                        ...
                        # 按钮按下时触发
                        if event.value == 1:
                            self.logger.debug(f"按钮 {control['name']} ({control.get('meaning', '')}) 按下")
                        ...
                        # 按钮释放
                        elif event.value == 0 and control["name"] in ["Gear_Forward", "Gear_Push_Back"]:
                            with self.wheel_lock:
                                self.wheel_values[control["name"]] = event.value
        except Exception as e:
            self.logger.error(f"方向盘事件监听线程出错: {e}")
        finally:
            self.logger.debug("方向盘事件监听线程结束")

    def update_wheel_control(self):
        """基于方向盘和踏板位置更新控制值"""
        if self.mode != self.MODE_WHEEL:
            return
    ...
    def switch_to_wheel_mode(self):
        ...
        with self.mode_lock:
            if self.mode != self.MODE_WHEEL:
                ...
                self.logger.info("\n\n")
                self.logger.info("****切换到 WHEEL 控制模式 (方向盘控制)****\n\n")
                # 重置命令
                self.send_stop_command()
    ...
    def switch_to_auto_mode(self):
        ...
        with self.mode_lock:
            if self.mode != self.MODE_AUTO:
                self.mode = self.MODE_AUTO
                self.logger.info("\n\n")
                self.logger.info("****切换到 AUTO 控制模式 (Autoware控制)****\n\n")
                # 重置命令
                self.send_stop_command()
                self.last_sent_commands['speed'] = None
                self.last_sent_commands['steering_angle_deg'] = None
    ...
    def toggle_mode(self):
        ...
        with self.mode_lock:
            if self.mode == self.MODE_AUTO:
                self.mode = self.MODE_MANUAL
                self.logger.info("\n\n")
                self.logger.info("****切换到 MANUAL 模式 (键盘控制)****\n\n")
                # 重置上次发送的命令
                self.last_sent_commands['steering_angle_deg'] = None
                self.last_sent_commands['speed'] = None
            else:
