...
class IntegratedControlPublisher(Node):
    ...
    def handle_enter_button(self):
        ...
        with self.wheel_lock:
            ...
            if self.mode == self.MODE_WHEEL:
                ...
                if brake_mapped > 0.05: # 刹车优先
                    current_speed = -brake_mapped * 1.0
                elif accel_mapped > 0.05: # 油门
                    current_speed = accel_mapped * 1.8
    ...
    def update_wheel_control(self, force_send=False):
        ...
        with self.wheel_lock:
            ...
            # 使用更小的阈值确保油门和刹车有效
            if brake_mapped > 0.05: # 刹车优先
                current_speed = -brake_mapped * 1.0 # 刹车产生负速度
                # 仅在超过阈值的情况下记录刹车动作
                ...
                current_speed = accel_mapped * 1.8 # 油门产生正速度
                # 仅在超过阈值的情况下记录油门动作
        ...
        if value_changed or force_send:
            if value_changed: # 仅在实际变化时记录
                # 调整方向盘控制日志的输出级别
                log_message = f"方向盘控制更新: 角={current_steering_angle}°, 速={current_speed}m/s, 档位={self.gear_direction}, 喇叭={'开' if self.trumpet_active else '关'}"
