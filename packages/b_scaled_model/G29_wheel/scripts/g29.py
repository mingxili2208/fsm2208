...
class IntegratedControlPublisher(Node):
    ...
    def __init__(self):
        ...
        self.EVENT_CODE_TO_KEY = {
            ecodes.EV_ABS: {
                ecodes.ABS_X: {"name": "wheel", "func": self.map_wheel}, # 通常ABS_X为方向盘
                ecodes.ABS_GAS: {"name": "Accelerater", "func": self.map_accelerator}, # 或ABS_RZ
                ecodes.ABS_BRAKE: {"name": "Brake", "func": self.map_brake},       # 或ABS_Z
            },
            ecodes.EV_KEY: {
                # 用evtest验证G29的这些按钮代码
                ecodes.BTN_TR: {"name": "Right_pick", "meaning": "右转向"}, # 例如: BTN_TR (R1/RB)
                ecodes.BTN_TL: {"name": "Left_pick", "meaning": "左转向"},  # 例如: BTN_TL (L1/LB)
                ecodes.BTN_SELECT: {"name": "ENTER", "meaning": "喇叭/蜂鸣器", "func": self.handle_enter_button}, # 或其他按钮
        ...
        try:
            for event in self.wheel_device.read_loop():
                ...
                if event.type == ecodes.EV_KEY and event.value == 1:
                    self.logger.debug(f"按钮事件: 代码={event.code}, 状态=按下")
                
                if event.type in self.EVENT_CODE_TO_KEY and event.code in self.EVENT_CODE_TO_KEY[event.type]:
                    control = self.EVENT_CODE_TO_KEY[event.type][event.code]
                    if event.type == ecodes.EV_ABS:
                        with self.wheel_lock:
                            if self.wheel_values.get(control["name"]) != event.value:
                                self.wheel_values[control["name"]] = event.value
                                # self.logger.debug(f"{control['name']} updated to {event.value}") # Debug
                        # 更新由main_loop的周期调用update_wheel_control发送
                    elif event.type == ecodes.EV_KEY:
                        if event.value == 1: # 按钮按下
                            self.logger.info(f"按钮 {control['name']} ({control.get('meaning', '')}) 按下")
                            if "func" in control:
                                control["func"]()
                        elif event.value == 0: # 按钮释放
                            if control["name"] == "ENTER": # 特定处理喇叭释放
    ...
    def handle_enter_button(self):
        ...
        # 命令将由update_wheel_control或手动模式发送，反映新的喇叭状态
        # 或需要时立即发送命令:
        self.send_command_with_trumpet(self.steering_angle_deg, self.speed, True, force_send=True)
    ...
    def release_trumpet(self):
        ...
        with self.wheel_lock:
            self.trumpet_active = False
        # 命令将由update_wheel_control或手动模式发送
        self.send_command_with_trumpet(self.steering_angle_deg, self.speed, False, force_send=True)
    ...
    def send_command_with_trumpet(self, steering_angle_deg, speed, trumpet_active, force_send=False):
        ...
        try:
            ...
            hex_msg = " ".join(f"{byte:02X}" for byte in complete_msg)
            self.logger.info(f"串口发送(喇叭): 时间戳={current_time_ms}, 角={steering_angle_deg}, 速={speed}, 喇叭={'开' if trumpet_active else '关'}, HEX={hex_msg}")
    ...
    def update_wheel_control(self, force_send=False):
        ...
        with self.wheel_lock:
            ...
            current_speed = 0.0
            if brake_mapped > 0.05: # 刹车优先，设置一些死区
                current_speed = -brake_mapped * 1.0 # 最大刹车速度(调整系数)
            elif accel_mapped > 0.05: # 油门死区
                current_speed = accel_mapped * 1.8 # 最大加速速度(调整系数)
            
            current_speed *= self.gear_direction
            current_speed = round(max(-1.8, min(1.8, current_speed)), 3)
        ...
        value_changed = (self.last_sent_commands['speed'] != current_speed or 
                         self.last_sent_commands['steering_angle_deg'] != current_steering_angle)
        
        if value_changed or force_send:
            if value_changed: # 仅在实际变化时记录
                 self.logger.info(f"方向盘控制更新: 角={current_steering_angle}°, 速={current_speed}m/s, 档位={self.gear_direction}, 喇叭={'开' if self.trumpet_active else '关'}")
    ...
    def send_command(self, steering_angle_deg, speed, force_send=False):
        ...
        try:
            ...
            self.logger.info(f"串口发送({source_mode}): 时间戳={current_time_ms}, 角={steering_angle_deg}, 速={speed}, 喇叭={'开' if actual_trumpet_state else '关'}, HEX={hex_msg}")
    ...
    def process_decoding_status(self, data: bytes):
        ...
        try:
            ...
            self.logger.info(f"解码状态: Orig(S:{orig_steering:.2f}, V:{orig_speed:.2f}), " +
        ...
        try:
            ...
            if sync_type == 0x01: # ACK
                with self.time_sync_lock:
                    ...
                    # 对于RTT: current_pc_time - self.last_sync_time (其中last_sync_time是请求发送时间)
    ...
    def process_ack_response(self, data: bytes):
        ...
        try:
            ...
            self.logger.info(f"{ack_type_str}确认 - PC命令时间戳: {pc_timestamp_echo}, Arduino确认时间: {arduino_time_ack}, 总延迟估算: {processing_delay} ms")
                ...
                if len(response) == 11 and response[0] == 0x42: # Arduino状态消息
                    ...
                    if checksum == response[10]:
                        ...
                        if msg_flag == 0x00: # NRF初始化失败
                            ...
                            self.logger.info(f"{log_msg_prefix}: NRF发送成功 - 映射角={val1:.2f}, 映射速={val2:.2f}")
