						...
						if(state>9)
						{
								if(state<190)
								{
										...
										else if(state==194)
										{
													...
													{
															data_process3();  	 /* 数据处理函数，完成一帧之后可进行数据处理 */
															receive_cnt3++;	 	 /* 输出接收到正确数据的次数 */
													}
													distance_2 = Pack_Data[0].distance;
													crc = 0;
													state = 0;
													state_flag = 1;
													cnt = 0; 							 /* 复位*/
...
void data_process3(void)/*数据处理函数，完成一帧之后可进行数据处理*/
{
		...
		if(count !=0)
		{
					...
					distance_2 = Pack_sum.distance/count;
...
//void data_trastion(void)
//{
//	Usart_Send_Data[0] = FRAME_HEADER;
//	Usart_Send_Data[1] = distance_1>>8;
//	Usart_Send_Data[2] = distance_1;
//	Usart_Send_Data[3] = distance_2>>8;
//	Usart_Send_Data[4] = distance_2;
//	Usart_Send_Data[5] = distance_3>>8;
//	Usart_Send_Data[6] = distance_3;
//	Usart_Send_Data[7] = distance_4>>8;
//	Usart_Send_Data[8] = distance_4;
//	Usart_Send_Data[9] = Check_Sum();
//	Usart_Send_Data[10] = FRAME_TALL;
//	
//	CAN_Send_Data[0] = distance_1>>8;
//	CAN_Send_Data[1] = distance_1;
//	CAN_Send_Data[2] = distance_2>>8;
//	CAN_Send_Data[3] = distance_2;
//	CAN_Send_Data[4] = distance_3>>8;
//	CAN_Send_Data[5] = distance_3;
//	CAN_Send_Data[6] = distance_4>>8;
//	CAN_Send_Data[7] = distance_4;
//}
