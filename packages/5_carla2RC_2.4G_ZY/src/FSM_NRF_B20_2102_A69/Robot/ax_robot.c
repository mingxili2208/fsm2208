...
{	
	...
	while(1)
	{
		//FSM_A69_SendRec(ax_control_mode);
		...
		if(ax_control_mode != 0)
		{
			...
			else if (ax_control_mode == CTL_A69)    {/*FSM_A69_SendRec(0x11);*/FSM_CTL_A69();}
		}
...
{	
	...
	{	
		...
		if(ax_control_mode)
		{
			...
			else if (ax_control_mode == CTL_NRF)    AX_OLED_DispStr(90, 2, "NRF", 0);   //SBUS航模遥控器控制
			else if (ax_control_mode == CTL_A69)    AX_OLED_DispStr(90, 2, "A69", 0);
		}
		else if (sampling_flag==0x01) 		AX_OLED_DispStr(90, 2, "Sampling", 0);
		else
		{
			//ROS控制模式
		...
		AX_OLED_DispValue(90,4,(distance_3),2,3,0);
		//显示电池电压，陀螺仪Z轴数据,
...
void A69_Task(void* parameter)
{
    ...
    while(1)
    {
        ...
        if (A69_DataAvailable())  
        {
            uint8_t receivedLength = A69_ReceiveData(rxBuffer, sizeof(rxBuffer));
            if (receivedLength > 0)
            {
                FSM_A69_ProcessReceivedData(rxBuffer, receivedLength,&nrt_ctl_info);
            }
						
        }
				if(ax_control_mode != CTL_A69)
				{
					
					if((nrt_ctl_info.ST == 0x01) )
					{
						//切换到A69模式
						ax_control_mode = CTL_A69;	
