...
u16 distance_2=0;
u16 distance_3=0; //d2--->u3;d3--->u4

u16 receive_cnt3,receive_cnt4;//计算成功接收数据帧次数
u8 confidence,count;
...
{	
	...
	uart3_init(230400);//lidar_f串口接收
	uart4_init(230400);//lidar_r串口接收
...
void Start_Task(void *pvParameters)
{
	...
	AX_OLED_DispStr(0, 4, " d1:00.000 d2:00.000", 0);
...
//			 (TaskHandle_t*  )&Nrf_Task_Handle);/* 任务控制块指针 */			
	xTaskCreate((TaskFunction_t )A69_Task, /* 任务入口函数 */
			 (const char*    )"A69_Task",/* 任务名字 */
			 (uint16_t       )A69_STK_SIZE,   /* 任务栈大小 */
			 (void*          )NULL,	/* 任务入口函数参数 */
			 (UBaseType_t    )A69_TASK_PRIO,	    /* 任务的优先级 */
			 (TaskHandle_t*  )&A69_Task_Handle);/* 任务控制块指针 */				 
