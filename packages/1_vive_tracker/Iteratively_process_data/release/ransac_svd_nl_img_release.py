...
ENHANCED RESIDUAL VECTOR VISUALIZATION
"""
import sys
import os
import datetime
...
RANSAC_INLIER_THRESHOLD = 0.04  # 内点阈值 40mm (0.040m)
...
class CoordinateTransformer:
    """
    Enhanced coordinate transformation with simple data splitting and comprehensive evaluation
    ENHANCED with improved residual vector visualization
    """
    ...
    def enhanced_registration(self):
        """Enhanced registration with simple data splitting and comprehensive evaluation"""
        if self.__positions_A is None or self.__positions_B is None:
            logger.error("Please provide position data first!")
            return None, None
        
        logger.info("=== Starting Enhanced Registration with Simple Data Splitting ===")
        ...
        # === 新增：可视化所有训练数据的SVD残差向量 ===
        self.residual_visualizer.visualize_residual_vectors_comprehensive(
            train_A, train_B, train_all_svd_transformed, 
            train_all_residual_vectors, "Training All Data SVD"
        )
        ...
        # 计算测试集SVD残差向量
        test_svd_residual_vectors = test_B - test_svd_transformed
        ...
        if success_all:
            ...
            # 计算修正后的残差向量
            test_final_residual_all = test_B - test_final_all
        ...
        if success_inliers:
            ...
            # 计算修正后的残差向量
            test_final_residual_inliers = test_B - test_final_inliers
        ...
        # Visualize SVD results on test set with enhanced residual vectors
        self.visualize_transformation_stage("Test SVD", test_A, test_B, test_svd_transformed, 
                                           test_svd_rmse, test_svd_residual_vectors)
        ...
        # === 新增：创建残差向量演化数据 ===
        points_history = [test_svd_transformed]
        residuals_history = [test_svd_residual_vectors]
        stage_names = ["Test SVD"]
        
        if test_final_all is not None:
            points_history.append(test_final_all)
            residuals_history.append(test_final_residual_all)
            stage_names.append("Test Final (All Data)")
        ...
        if test_final_inliers is not None:
            ...
            stage_names.append("Test Final (Inliers)")
    ...
    def _create_comparison_visualization(self, test_A, test_B, svd_result, final_all, final_inliers, 
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(img_dir, 'comprehensive_comparison_enhanced.png'), dpi=300, bbox_inches='tight')
        plt.close()
        
        logger.info("Enhanced comparison visualization saved")
...
def main():
    ...
    logger.info("Features: Simple data splitting + RANSAC + SVD + Dual deformation learning + Test evaluation")
    logger.info("ENHANCED: Comprehensive residual vector visualization")
    ...
    if T_svd is not None and final_rmse_results is not None:
        ...
        with open(summary_file, "w") as f:
            f.write("Enhanced Registration Summary\n")
            f.write("=" * 80 + "\n")
            f.write("Features: Simple data splitting + RANSAC + SVD + Dual deformation learning + Test evaluation\n")
            f.write("ENHANCED: Comprehensive residual vector visualization\n")
            ...
            f.write(f"\nMethod: Simple random data splitting + RANSAC outlier removal + SVD registration + Dual RBF deformation learning\n")
    ...
    logger.info("Enhanced processing complete with improved residual vector visualization")
