...
class ExperimentRunner:
    ...
    def run_cpd(self, train_A, train_B, test_A, test_B):
        """Method 3: Coherent Point Drift (CPD) - UPDATED with optimized parameters"""
        ...
        try:
            ...
            if hasattr(cpd_reg, '__call__'):
                # If cpd_reg is a function, this part might need adjustment if it's the one being used
                try:
                    # MODIFIED: Apply optimized parameters here as well if this branch is active
                    reg = cpd_reg(X=train_B_xz, Y=train_A_aligned_xz, max_iterations=50, tolerance=1e-4, w=0.1, alpha=1.0, beta=2.0)
                except TypeError:
                     # Fallback for different function signatures
                    reg = cpd_reg(train_B_xz, train_A_aligned_xz)
            else:
                # If cpd_reg is a class (most likely case for pycpd)
                try:
                    # =================================================================
                    # MODIFIED LINE: Applied optimized parameters from your results
                    # =================================================================
                    reg = cpd_reg(X=train_B_xz, Y=train_A_aligned_xz, max_iterations=50, tolerance=1e-4, w=0.1, alpha=1.0, beta=2.0)
                    logger.info("CPD initialized with optimized parameters: max_iter=50, tol=1e-4, w=0.1, alpha=1.0, beta=2.0")

                except Exception as e_init:
                    logger.warning(f"CPD initialization with optimized parameters failed: {e_init}. Trying fallback.")
                    try:
                        # Fallback to original parameters if the new ones cause an error
            ...
            if hasattr(reg, 'transform_point_cloud'):
                ...
                # This is a tricky case. TY is the transformed source cloud from training.
                # A proper transform on new points is needed.
                # This fallback might not be correct for new test data.
                logger.warning("CPD object does not have a dedicated transform method for new data. Results may be inaccurate.")
    ...
    def _create_comprehensive_plots(self, results, final_points):
        """
        Create publication-quality comparison plots with English labels only
        """
        logger.info("Generating publication-quality comparison plots...")
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        # Fallback to a basic clean style
        ...
        # Figure 1: Main Performance Comparison (2x2 layout)
        fig1, axes1 = plt.subplots(2, 2, figsize=(12, 10))
        fig1.suptitle('Registration Methods Performance Benchmark', fontsize=18, fontweight='bold')
        
        # Subplot 1A: Final Test Accuracy (Bar chart)
        ax = axes1[0, 0]
        ...
        for i, bar in enumerate(bars):
            if not np.isnan(test_rmse_values[i]):
                if test_rmse_values[i] <= TARGET_RMSE * 1000:
                    bar.set_color('tab:green')  # Green for meeting target
                else:
                    bar.set_color('tab:red')   # Red for not meeting target
        
        ax.set_ylabel('Test RMSE (mm)', fontsize=12)
        ax.set_title('A) Final Test Accuracy', fontsize=14, fontweight='bold')
        ax.tick_params(axis='x', rotation=45, labelsize=10)
        ax.tick_params(axis='y', labelsize=10)
        ax.grid(True, alpha=0.3)
        ax.legend()

        # Subplot 1B: Accuracy vs Efficiency Trade-off (Scatter plot)
        ax = axes1[0, 1]
        valid_indices = ~np.isnan(test_rmse_values)
        valid_times = np.array(time_values)[valid_indices]
        valid_rmse = np.array(test_rmse_values)[valid_indices]
        valid_methods = np.array(methods)[valid_indices]

        ax.scatter(valid_times, valid_rmse, s=120, alpha=0.8, c=valid_rmse, cmap='viridis_r', zorder=10)
        ax.axhline(y=TARGET_RMSE * 1000, color='red', linestyle='--', linewidth=2, label=f'Target: {TARGET_RMSE*1000:.1f}mm')
        
        texts = []
        for i, method in enumerate(valid_methods):
            texts.append(ax.text(valid_times[i], valid_rmse[i], method, fontsize=10))
        
        # Use adjust_text to avoid label overlap if available
        ...
        ax.set_ylabel('Test RMSE (mm)', fontsize=12)
        ax.set_title('B) Accuracy vs Efficiency Trade-off', fontsize=14, fontweight='bold')
        ax.tick_params(labelsize=10)
        ax.grid(True, alpha=0.3)
        ax.legend()
        
        # Subplot 1C: Overfitting Analysis (Scatter plot)
        ax = axes1[1, 0]
        ...
        ax.scatter(valid_train, valid_test, s=120, alpha=0.8, c=valid_test, cmap='viridis_r', zorder=10)
        ...
        ax.set_ylabel('Test RMSE (mm)', fontsize=12)
        ax.set_title('C) Generalization Analysis', fontsize=14, fontweight='bold')
        ax.tick_params(labelsize=10)
        ax.grid(True, alpha=0.3)
        ax.legend()
        ...
        fig1.savefig(os.path.join(img_dir, 'figure1_main_results.pdf'), dpi=300, bbox_inches='tight')
        fig1.savefig(os.path.join(img_dir, 'figure1_main_results.png'), dpi=300, bbox_inches='tight')
        plt.close(fig1)
        logger.info("Saved Figure 1: Main Performance Results")
        ...
        ax2.set_title('Detailed Performance Metrics Heatmap (mm)', fontsize=16, fontweight='bold')
        ...
        for i in range(len(methods)):
            for j in range(len(metrics_names)):
                if not np.isnan(performance_matrix[i, j]):
                    ...
                    ax2.text(j, i, f'{val:.2f}', ha="center", va="center", color=color, fontsize=11, fontweight='medium')

        cbar = fig2.colorbar(im, ax=ax2)
        cbar.set_label('Error Magnitude (mm)', fontsize=12)
        
        plt.tight_layout()
        fig2.savefig(os.path.join(img_dir, 'figure2_detailed_heatmap.pdf'), dpi=300, bbox_inches='tight')
        fig2.savefig(os.path.join(img_dir, 'figure2_detailed_heatmap.png'), dpi=300, bbox_inches='tight')
        plt.close(fig2)
        logger.info("Saved Figure 2: Detailed Performance Heatmap")
