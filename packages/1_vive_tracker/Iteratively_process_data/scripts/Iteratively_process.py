...
Processes existing laser tracker data to calculate coordinate transformations
"""
import sys
import os
import datetime
...
def calculate_transformations_iterative(max_iterations=10, target_rmse=0.03):
    """
    Iteratively calculate transformation matrices, filtering out high-error points each iteration,
    until target RMSE is achieved or max iterations reached.
    """
    logger.info(f"Starting iterative transformation calculation (target RMSE: {target_rmse})")
    ...
    while current_rmse > target_rmse and iteration <= max_iterations:
        ...
        try:
            ...
            # Demo transformation to get results
            transformed_positions, transformed_orientations, error_results = transformer.demo_transformation()
            
            # Get current RMSE
            current_rmse = error_results['position_rmse']
            logger.info(f"Current RMSE: {current_rmse:.6f}")
            
            # Get transformation matrices
            T_pos, R_euler = transformer.get_transformation_matrix()
            
            # If we've reached target RMSE, we're done
            if current_rmse <= target_rmse:
                logger.info(f"Target RMSE of {target_rmse} achieved! Stopping iterations.")
                break
            
            # Otherwise, filter out high error points for next iteration
            error_threshold = max(0.9 * current_rmse, 0.01)  # Dynamically adjust threshold
            logger.info(f"Filtering points with error > {error_threshold:.6f}")
            
            # Visualize and get error indices
            ...
            # Also save transformation matrices for this iteration
            np.savetxt(os.path.join(result_dir, f"T_pos_iter_{iteration}.txt"), T_pos)
            np.savetxt(os.path.join(result_dir, f"R_euler_iter_{iteration}.txt"), R_euler)
            ...
            # If we've removed too many points, stop
            if len(filtered_df) < 10:  # Arbitrary minimum to ensure statistical validity
    ...
    logger.info("\n--- Final Results ---")
    logger.info(f"Final RMSE: {current_rmse:.6f}")
    logger.info(f"Iterations performed: {iteration - 1}")
    logger.info(f"Final number of points used: {len(tracker_laser_df)}")
    
    # Save final transformation matrices
    final_output_path = os.path.join(output_dir, "final_transformation_matrices.txt")
    with open(final_output_path, "w") as f:
        f.write(f"Final RMSE: {current_rmse:.6f}\n")
    ...
    logger.info(f"Final transformation matrices saved to {final_output_path}")
    
    return T_pos, R_euler, current_rmse

def calculate_transformations():
    """Calculate transformation matrices from existing data files"""
    try:
...
def main():
    """Main function to process tracker data"""
    logger.info("Starting tracker data processing")
    
    global LASER_TRACKER_CSV, EULER_CSV
    if len(sys.argv) > 1:
        LASER_TRACKER_CSV = sys.argv[1]
    if len(sys.argv) > 2:
        EULER_CSV = sys.argv[2]
    ...
    if os.path.exists(LASER_TRACKER_CSV):
        ...
        if os.path.exists(EULER_CSV) and os.path.exists(CORRECTED_LASER_TRACKER_CSV):
            ...
            if T_pos is not None and R_euler is not None:
                if final_rmse <= target_rmse:
                    logger.info(f"Transformation calculation completed successfully with RMSE {final_rmse:.6f} (below target {target_rmse})")
                else:
                    logger.info(f"Transformation calculation completed, but RMSE {final_rmse:.6f} is above target {target_rmse}")
            else:
                logger.error("Transformation calculation failed")
        else:
            logger.error(f"Missing files for transformation calculation. "
        ...
        logger.info("Please provide the path to laser_tracker.csv as the first command line argument")
        logger.info("Usage: python script.py <path_to_laser_tracker.csv> [<path_to_euler.csv>]")
