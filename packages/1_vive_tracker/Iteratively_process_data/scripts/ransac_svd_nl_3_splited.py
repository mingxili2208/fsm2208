...
With train/test split for evaluation
"""
import sys
import os
import datetime
...
output_dir = os.path.join(result_dir, f"RANSAC_SVD_Deformation_TrainTest_{timestamp}_1to32_scale_RMSE_{TARGET_RMSE*1000:.1f}mm_{csv_filename}")
...
class RANSACFilter:
    ...
    def visualize_ransac_results(self, original_A, original_B, inlier_A, inlier_B, outlier_A, outlier_B):
        """可视化RANSAC结果"""
        ...
        ax1.set_ylabel('Z [m]')
        ax1.set_zlabel('Y [m]')
        ax1.set_title(f'Before RANSAC (Total: {len(original_A)} points)')
        ax1.legend()
        ...
        ax2.set_ylabel('Z [m]')
        ax2.set_zlabel('Y [m]')
        ax2.set_title(f'After RANSAC - Inliers ({len(inlier_A)} points)')
        ax2.legend()
        ...
        ax3.set_ylabel('Z [m]')
        ax3.set_zlabel('Y [m]')
        ax3.set_title(f'Removed Outliers ({len(outlier_A)} points)')
        ax3.legend()
        ...
        ax4.set_title(f'XZ Plane - Before RANSAC ({len(original_A)} points)')
        ax4.legend()
        ...
        ax5.set_title(f'XZ Plane - Inliers ({len(inlier_A)} points)')
        ax5.legend()
        ...
        ax6.set_title(f'XZ Plane - Outliers ({len(outlier_A)} points)')
        ax6.legend()
        ax6.grid(True, alpha=0.3)
        ax6.axis('equal')
        
        plt.tight_layout()
        plt.savefig(os.path.join(img_dir, 'ransac_filtering_results.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_ransac_statistics(self):
        ...
        ax1.set_title('RANSAC Inlier Count per Iteration')
        ax1.legend()
        ...
        ax2.set_title('RANSAC Inlier Ratio per Iteration')
        ax2.legend()
        ...
        ax3.set_title('RANSAC Mean Inlier Distance per Iteration')
        ax3.legend()
    ...
    def save_ransac_results(self, original_A, original_B, inlier_A, inlier_B, outlier_A, outlier_B, original_df=None):
        """保存RANSAC结果"""
        # 保存内点
        if len(inlier_A) > 0:
            inlier_data = np.hstack([inlier_A, inlier_B])
            np.savetxt(os.path.join(result_dir, "ransac_inliers.txt"), inlier_data, 
                      header="source_x source_y source_z target_x target_y target_z")
        
        # 保存外点
        if len(outlier_A) > 0:
            outlier_data = np.hstack([outlier_A, outlier_B])
            np.savetxt(os.path.join(result_dir, "ransac_outliers.txt"), outlier_data, 
                      header="source_x source_y source_z target_x target_y target_z")
        
        # 生成内点CSV文件（新增功能）
        if len(inlier_A) > 0 and original_df is not None:
            self.save_inlier_csv(original_A, original_B, inlier_A, inlier_B, original_df)
        
        # 保存RANSAC参数和结果摘要
        with open(os.path.join(result_dir, "ransac_summary.txt"), "w") as f:
            f.write("RANSAC Filtering Summary\n")
            f.write("=" * 30 + "\n")
            ...
            f.write(f"\nResults:\n")
            f.write(f"  - Original points: {len(original_A)}\n")
            f.write(f"  - Inliers: {len(inlier_A)} ({len(inlier_A)/len(original_A)*100:.1f}%)\n")
            f.write(f"  - Outliers: {len(outlier_A)} ({len(outlier_A)/len(original_A)*100:.1f}%)\n")
            if self.best_transformation:
                f.write(f"  - Best transformation found: Yes\n")
            else:
                f.write(f"  - Best transformation found: No\n")
    
    def save_inlier_csv(self, original_A, original_B, inlier_A, inlier_B, original_df):
        """保存内点的CSV文件"""
        logger.info("Generating inlier CSV file...")
        ...
        for i, (inlier_a, inlier_b) in enumerate(zip(inlier_A, inlier_B)):
            # 在原始数据中找到匹配的点对
            for j, (orig_a, orig_b) in enumerate(zip(original_A, original_B)):
                # 检查是否为同一个点对（使用小的容差）
                if (np.allclose(inlier_a, orig_a, atol=1e-10) and 
                    np.allclose(inlier_b, orig_b, atol=1e-10)):
                    inlier_indices.append(j)
                    break
        
        # 创建内点数据框
        if len(inlier_indices) > 0:
            inlier_df = original_df.iloc[inlier_indices].copy()
            inlier_df.reset_index(drop=True, inplace=True)
            
            # 添加RANSAC标记列
            inlier_df['ransac_inlier'] = True
            inlier_df['ransac_index'] = range(len(inlier_df))
            
            # 保存为CSV
            inlier_csv_path = os.path.join(result_dir, "ransac_inlier_data.csv")
            inlier_df.to_csv(inlier_csv_path, index=False)
            
            logger.info(f"Inlier CSV saved: {inlier_csv_path}")
            logger.info(f"Inlier CSV contains {len(inlier_df)} rows")
            
            # 保存列信息
            with open(os.path.join(result_dir, "inlier_csv_info.txt"), "w") as f:
                f.write("RANSAC Inlier CSV Information\n")
                f.write("=" * 40 + "\n")
                f.write(f"File: ransac_inlier_data.csv\n")
                f.write(f"Total rows: {len(inlier_df)}\n")
                f.write(f"Columns: {list(inlier_df.columns)}\n")
                f.write(f"Original dataset rows: {len(original_df)}\n")
                f.write(f"Inlier ratio: {len(inlier_df)/len(original_df)*100:.2f}%\n")
                f.write(f"\nColumn descriptions:\n")
                f.write(f"  - ransac_inlier: Always True for this file\n")
                f.write(f"  - ransac_index: Sequential index in inlier dataset\n")
                f.write(f"  - Other columns: Same as original dataset\n")
        else:
            logger.warning("No inlier indices found - CSV not generated")
...
class CoordinateTransformer:
    """
    Three-stage coordinate transformation: RANSAC filtering + SVD coarse + Non-rigid deformation learning
    With train/test split for proper evaluation
    """
    def __init__(self, positions_A=None, positions_B=None, original_df=None):
        ...
        self.__filtered_positions_A = None
        self.__filtered_positions_B = None
    ...
    def split_train_test(self, inlier_A, inlier_B):
        """
        Split inliers into training and testing sets
        """
        logger.info("=== Splitting Data into Train/Test Sets ===")
        
        inlier_A = np.array(inlier_A)
        inlier_B = np.array(inlier_B)
        
        # Perform train/test split
        train_A, test_A, train_B, test_B, train_indices, test_indices = train_test_split(
            inlier_A, inlier_B, np.arange(len(inlier_A)),
            test_size=(1 - TRAIN_TEST_SPLIT_RATIO),
            random_state=RANDOM_STATE,
            shuffle=True
        )
        
        self.__train_A = train_A
        self.__train_B = train_B
        self.__test_A = test_A
        self.__test_B = test_B
        self.__train_indices = train_indices
        self.__test_indices = test_indices
        
        logger.info(f"Data split completed:")
        logger.info(f"  - Total inliers: {len(inlier_A)}")
        logger.info(f"  - Training set: {len(train_A)} points ({len(train_A)/len(inlier_A)*100:.1f}%)")
        logger.info(f"  - Testing set: {len(test_A)} points ({len(test_A)/len(inlier_A)*100:.1f}%)")
        logger.info(f"  - Random state: {RANDOM_STATE}")
        
        # Save split information
        self._save_train_test_split()
        
        # Visualize split
        self._visualize_train_test_split(inlier_A, inlier_B)
        
        return train_A, train_B, test_A, test_B

    def _save_train_test_split(self):
        """Save train/test split data"""
        # Save training set
        if len(self.__train_A) > 0:
            train_data = np.hstack([self.__train_A, self.__train_B])
            np.savetxt(os.path.join(result_dir, "train_data.txt"), train_data, 
                      header="source_x source_y source_z target_x target_y target_z")
            
        # Save testing set
        if len(self.__test_A) > 0:
            test_data = np.hstack([self.__test_A, self.__test_B])
        ...
        # Save split summary
        with open(os.path.join(result_dir, "train_test_split_summary.txt"), "w") as f:
            ...
            f.write(f"Total inliers: {len(self.__train_A) + len(self.__test_A)}\n")
            f.write(f"Training points: {len(self.__train_A)}\n")
            f.write(f"Testing points: {len(self.__test_A)}\n")
            f.write(f"Training percentage: {len(self.__train_A)/(len(self.__train_A) + len(self.__test_A))*100:.2f}%\n")
            f.write(f"Testing percentage: {len(self.__test_A)/(len(self.__train_A) + len(self.__test_A))*100:.2f}%\n")

    def _visualize_train_test_split(self, inlier_A, inlier_B):
        ...
        ax1.scatter(self.__train_A[:, 0], self.__train_A[:, 2], self.__train_A[:, 1], 
                   c='blue', marker='o', s=40, label=f'Train Source ({len(self.__train_A)})', alpha=0.8)
        ax1.scatter(self.__train_B[:, 0], self.__train_B[:, 2], self.__train_B[:, 1], 
                   c='red', marker='^', s=40, label=f'Train Target ({len(self.__train_B)})', alpha=0.8)
        
        # Plot testing points
        ax1.scatter(self.__test_A[:, 0], self.__test_A[:, 2], self.__test_A[:, 1], 
                   c='lightblue', marker='s', s=40, label=f'Test Source ({len(self.__test_A)})', alpha=0.8)
        ax1.scatter(self.__test_B[:, 0], self.__test_B[:, 2], self.__test_B[:, 1], 
                   c='orange', marker='d', s=40, label=f'Test Target ({len(self.__test_B)})', alpha=0.8)
        
        ax1.set_xlabel('X [m]')
        ax1.set_ylabel('Z [m]')
        ax1.set_zlabel('Y [m]')
        ax1.set_title('3D View - Train/Test Split')
        ax1.legend()
        ax1.set_zlim(-0.1, 0.1)
        ...
        # Plot training points
        ax2.scatter(self.__train_A[:, 0], self.__train_A[:, 2], 
                   c='blue', marker='o', s=40, label=f'Train Source ({len(self.__train_A)})', alpha=0.8)
        ax2.scatter(self.__train_B[:, 0], self.__train_B[:, 2], 
                   c='red', marker='^', s=40, label=f'Train Target ({len(self.__train_B)})', alpha=0.8)
        
        # Plot testing points
        ax2.scatter(self.__test_A[:, 0], self.__test_A[:, 2], 
                   c='lightblue', marker='s', s=40, label=f'Test Source ({len(self.__test_A)})', alpha=0.8)
        ax2.scatter(self.__test_B[:, 0], self.__test_B[:, 2], 
                   c='orange', marker='d', s=40, label=f'Test Target ({len(self.__test_B)})', alpha=0.8)
        
        ax2.set_xlabel('X (Lateral) [m]')
        ax2.set_ylabel('Z (Longitudinal) [m]')
        ax2.set_title('XZ Plane - Train/Test Split')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ax2.axis('equal')
        ...
        ax3.hist(self.__train_A[:, 0], bins=20, alpha=0.5, label='Train X', color='blue')
        ax3.hist(self.__test_A[:, 0], bins=20, alpha=0.5, label='Test X', color='orange')
        ...
        ax4.hist(self.__train_A[:, 2], bins=20, alpha=0.5, label='Train Z', color='blue')
        ax4.hist(self.__test_A[:, 2], bins=20, alpha=0.5, label='Test Z', color='orange')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(img_dir, 'train_test_split_visualization.png'), dpi=300, bbox_inches='tight')
        plt.close()
        
        logger.info("Train/test split visualization saved")
    ...
    def calculate_svd_transformation(self, positions_A=None, positions_B=None):
        ...
        if positions_A is not None and positions_B is not None:
            ...
            train_A = self.__train_A
            train_B = self.__train_B
        ...
        logger.info(f"SVD transformation matrix calculated using {len(train_A)} training points")
    ...
    def three_stage_registration(self):
        """Perform three-stage registration with train/test split evaluation"""
        if self.__positions_A is None or self.__positions_B is None:
            logger.error("Please provide position data first!")
            return None, None
        
        logger.info("=== Starting Three-Stage Registration with Train/Test Split ===")
        
        original_positions_A = np.array(self.__positions_A)
        original_positions_B = np.array(self.__positions_B)
        
        # Stage 1: RANSAC data filtering
        logger.info("Stage 1: RANSAC Data Filtering")
        ...
        inlier_A, inlier_B, outlier_A, outlier_B = self.ransac_filter.ransac_filter(
        ...
        self.ransac_filter.visualize_ransac_results(
            original_positions_A, original_positions_B, 
            inlier_A, inlier_B, outlier_A, outlier_B
        )
        
        # 保存RANSAC结果
        self.ransac_filter.save_ransac_results(
            original_positions_A, original_positions_B,
            inlier_A, inlier_B, outlier_A, outlier_B, self.__original_df
        )
        
        # Store filtered data
        self.__filtered_positions_A = inlier_A
        self.__filtered_positions_B = inlier_B
        
        # Stage 1.5: Split into train/test sets
        logger.info("Stage 1.5: Splitting Data into Train/Test Sets")
        train_A, train_B, test_A, test_B = self.split_train_test(inlier_A, inlier_B)
        
        # Stage 2: SVD coarse registration (using training data only)
        logger.info("Stage 2: SVD Coarse Registration (Training on Train Set)")
        self.calculate_svd_transformation(train_A, train_B)
        
        # Apply SVD transformation to training set
        svd_transformed_train = self.apply_transformation(train_A, self.T_svd)
        svd_rmse_train = self.calculate_directional_rmse(train_B, svd_transformed_train)
        
        logger.info(f"SVD Training RMSE: {svd_rmse_train['overall_rmse']*1000:.3f}mm")
        
        # Apply SVD transformation to test set
        svd_transformed_test = self.apply_transformation(test_A, self.T_svd)
        svd_rmse_test = self.calculate_directional_rmse(test_B, svd_transformed_test)
        
        logger.info(f"SVD Testing RMSE: {svd_rmse_test['overall_rmse']*1000:.3f}mm")
        
        # Calculate residual vectors for training data
        residual_vectors_train = train_B - svd_transformed_train
        logger.info(f"Calculated training residual vectors for {len(residual_vectors_train)} points")
        
        # Visualize SVD results for both sets
        self.visualize_transformation_stage("SVD Coarse Registration", 
                                           train_A, train_B, svd_transformed_train, 
                                           svd_rmse_train, residual_vectors_train, is_test=False)
        
        self.visualize_transformation_stage("SVD Coarse Registration", 
                                           test_A, test_B, svd_transformed_test, 
                                           svd_rmse_test, None, is_test=True)
        
        # Stage 3: Learn non-rigid deformation (using training data only)
        logger.info("Stage 3: Learning Non-rigid Deformation (Training on Train Set)")
        ...
        final_rmse_train = self.calculate_directional_rmse(train_B, final_transformed_train)
        ...
        final_rmse_test = self.calculate_directional_rmse(test_B, final_transformed_test)
        logger.info(f"Final Testing RMSE: {final_rmse_test['overall_rmse']*1000:.3f}mm")
        
        # Calculate final residuals
        final_residuals_train = train_B - final_transformed_train
        final_residuals_test = test_B - final_transformed_test
        
        # Visualize final results for both sets
        self.visualize_transformation_stage("Non-rigid Deformation Final", 
                                           train_A, train_B, final_transformed_train, 
                                           final_rmse_train, final_residuals_train, is_test=False)
        
        self.visualize_transformation_stage("Non-rigid Deformation Final", 
                                           test_A, test_B, final_transformed_test, 
        ...
        self._print_detailed_results_with_split(
            len(original_positions_A), len(inlier_A),
    ...
    def _create_train_test_comparison(self, svd_train, svd_test, final_train, final_test):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(img_dir, 'train_test_performance_comparison.png'), dpi=300, bbox_inches='tight')
        plt.close()
        
        logger.info("Train/test performance comparison saved")

    def _print_detailed_results_with_split(self, original_count, filtered_count, train_count, test_count,
                                         svd_train, svd_test, final_train, final_test):
        """Print detailed performance analysis with train/test split"""
        logger.info("\n=== Detailed Performance Analysis with Train/Test Split ===")
        
        logger.info("Data Split Summary:")
        logger.info(f"  Original points: {original_count}")
        logger.info(f"  After RANSAC: {filtered_count} ({filtered_count/original_count*100:.1f}%)")
        logger.info(f"  Training set: {train_count} ({train_count/filtered_count*100:.1f}%)")
        logger.info(f"  Testing set: {test_count} ({test_count/filtered_count*100:.1f}%)")
        ...
        logger.info(f"  Testing target achieved: {'✓' if test_pass else '✗'}")
    ...
    def _save_transformation_data(self):
        ...
        if self.deformation_model is not None and self.deformation_model.source_points is not None:
            ...
            with open(os.path.join(result_dir, "deformation_model_info.txt"), "w") as f:
                ...
                f.write(f"Training set size: {len(self.__train_A) if self.__train_A is not None else 'N/A'}\n")
                f.write(f"Testing set size: {len(self.__test_A) if self.__test_A is not None else 'N/A'}\n")
...
def main():
    """Main function"""
    logger.info("Starting 1:32 scale model tracker data processing with RANSAC + SVD + Non-rigid Deformation (with Train/Test Split)")
    ...
    # Perform three-stage registration with train/test split
    transformer = CoordinateTransformer(positions_A, positions_B, df)
    T_svd, final_rmse_test = transformer.three_stage_registration()
    
    if T_svd is not None and final_rmse_test is not None:
        # Success is determined by test set performance
        success = final_rmse_test['overall_rmse'] <= TARGET_RMSE
        if success:
            logger.info(f"SUCCESS: Registration completed with test RMSE {final_rmse_test['overall_rmse']*1000:.3f}mm")
        else:
            logger.info(f"PARTIAL: Registration completed, but test RMSE {final_rmse_test['overall_rmse']*1000:.3f}mm exceeds target {TARGET_RMSE*1000:.2f}mm")
        
        # Save final summary
        summary_file = os.path.join(output_dir, "registration_summary.txt")
        with open(summary_file, "w") as f:
            f.write("Three-Stage Registration Summary with Train/Test Split\n")
            f.write("=" * 60 + "\n")
            ...
            f.write(f"Test Set RMSE: {final_rmse_test['overall_rmse']*1000:.3f}mm\n")
            f.write(f"Success: {'Yes' if success else 'No'}\n")
            f.write(f"\nData Split:\n")
            f.write(f"  Train/Test ratio: {TRAIN_TEST_SPLIT_RATIO:.1f}/{1-TRAIN_TEST_SPLIT_RATIO:.1f}\n")
            f.write(f"  Random state: {RANDOM_STATE}\n")
            f.write(f"  Original points: {len(positions_A)}\n")
            f.write(f"  After RANSAC: {len(transformer._CoordinateTransformer__filtered_positions_A) if hasattr(transformer, '_CoordinateTransformer__filtered_positions_A') and transformer._CoordinateTransformer__filtered_positions_A is not None else 'N/A'}\n")
            f.write(f"  Training points: {len(transformer._CoordinateTransformer__train_A) if hasattr(transformer, '_CoordinateTransformer__train_A') and transformer._CoordinateTransformer__train_A is not None else 'N/A'}\n")
            f.write(f"  Testing points: {len(transformer._CoordinateTransformer__test_A) if hasattr(transformer, '_CoordinateTransformer__test_A') and transformer._CoordinateTransformer__test_A is not None else 'N/A'}\n")
            f.write(f"\nTest Set Performance (Final Evaluation):\n")
            f.write(f"  Real-world equivalent: {final_rmse_test['overall_rmse']*SCALE_FACTOR*100:.2f}cm\n")
            ...
            f.write(f"  Evaluation: Test set (unseen data)\n")
            f.write(f"\nOutput Files:\n")
            f.write(f"  - train_data.txt / test_data.txt: Split datasets\n")
            f.write(f"  - *_train_results.png / *_test_results.png: Performance visualizations\n")
            f.write(f"  - train_test_performance_comparison.png: Comparison analysis\n")
            f.write(f"  - ransac_inlier_data.csv: Filtered data for further use\n")
    else:
