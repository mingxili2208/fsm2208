...
# -*- coding: utf-8 -*-
"""
Tracker data processing script with SVD + Non-rigid Deformation Learning (Optimized with CV and hyperparameter tuning)
"""
import sys
import os
import datetime
...
class BSplineDeformationModel:
    """
    B-Spline based non-rigid deformation model using LSQBivariateSpline
    """
    ...
    def learn_deformation(self, source_points, residual_vectors):
        ...
        if self.constrain_y:
            ...
            try:
                ...
                logger.info(f"B-Spline deformation model learned with {self.knot_density}x{self.knot_density} control grid")
                return True
...
class HyperparameterOptimizer:
    ...
    def optimize_bspline_params(self, positions_A, positions_B, T_svd):
        ...
        param_grid = {
            ...
            'smoothing_factor': [0.01, 0.1, 0.5, 1.0]
        }
...
class CoordinateTransformer:
    """
    Optimized two-stage coordinate transformation with hyperparameter tuning
    """
    ...
    def two_stage_registration_optimized(self):
        """Perform optimized two-stage registration with hyperparameter tuning"""
        if self.__positions_A is None or self.__positions_B is None:
            logger.error("Please provide position data first!")
            return None, None
        
        logger.info("=== Starting Optimized Two-Stage Registration ===")
        ...
        logger.info("Stage 2: Hyperparameter Optimization and Model Selection")
        
        # 优化RBF参数
        best_rbf_params = self.optimizer.optimize_rbf_params(positions_A, positions_B, self.T_svd)
        
        # 优化B样条参数
        best_bspline_params, bspline_results = self.optimizer.optimize_bspline_params(positions_A, positions_B, self.T_svd)
        ...
        # 可视化最终结果
        self.visualize_transformation_stage(f"Final Optimized {best_model_info['type'].upper()}", 
    ...
    def _save_transformation_data(self):
        ...
        logger.info("Transformation data and model info saved")
...
def main():
    """Main function"""
    logger.info("Starting OPTIMIZED 1:32 scale model tracker data processing")
    logger.info(f"Scale factor: {SCALE_FACTOR}")
    logger.info(f"Target RMSE: {TARGET_RMSE*1000:.2f}mm")
    logger.info("Features: Cross-validation, hyperparameter optimization, model selection")
    ...
    # Perform optimized two-stage registration
    ...
    if T_svd is not None and final_rmse_results is not None:
        ...
        if success:
            logger.info(f"SUCCESS: Optimized registration completed with RMSE {final_rmse_results['overall_rmse']*1000:.3f}mm")
        else:
        ...
        with open(summary_file, "w") as f:
            f.write("Optimized Two-Stage Registration Summary\n")
            f.write("=" * 80 + "\n")
            ...
            f.write("- B-Spline parameter optimization\n")
