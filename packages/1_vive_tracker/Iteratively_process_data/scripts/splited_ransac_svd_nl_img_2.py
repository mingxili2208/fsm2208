...
Enhanced RANSAC outlier visualization
"""
import sys
import os
import datetime
...
class CoordinateTransformer:
    ...
    def create_ransac_outlier_analysis_visualization(self, train_inlier_A, train_inlier_B, train_outlier_A, train_outlier_B,
                                                    test_inlier_A, test_inlier_B, test_outlier_A, test_outlier_B):
        """
        Figure 2A: Enhanced RANSAC Outlier Analysis and Transformation Error Visualization
        """
        logger.info("Creating Figure 2A: Enhanced RANSAC Outlier Analysis...")
        
        # Apply the best RANSAC transformation to outliers to see their actual errors
        if self.ransac_filter.best_transformation:
            best_R, best_t = self.ransac_filter.best_transformation
            
            # Transform outliers using the best RANSAC model
            if len(train_outlier_A) > 0:
                transformed_train_outliers = self.ransac_filter.apply_2d_transformation(train_outlier_A, best_R, best_t)
                train_outlier_errors = np.sqrt((transformed_train_outliers[:, 0] - train_outlier_B[:, 0])**2 + 
                                              (transformed_train_outliers[:, 2] - train_outlier_B[:, 2])**2) * 1000
            else:
                transformed_train_outliers = np.array([]).reshape(0, 3)
                train_outlier_errors = np.array([])
                
            if len(test_outlier_A) > 0:
                transformed_test_outliers = self.ransac_filter.apply_2d_transformation(test_outlier_A, best_R, best_t)
                test_outlier_errors = np.sqrt((transformed_test_outliers[:, 0] - test_outlier_B[:, 0])**2 + 
                                             (transformed_test_outliers[:, 2] - test_outlier_B[:, 2])**2) * 1000
            else:
                transformed_test_outliers = np.array([]).reshape(0, 3)
                test_outlier_errors = np.array([])
        
            # Create comprehensive visualization
            fig = plt.figure(figsize=(24, 16))
            fig.suptitle('Figure 2A: Enhanced RANSAC Outlier Analysis and Transformation Error Visualization', 
                         fontsize=18, fontweight='bold')
            
            # (a) Training set outliers with error vectors
            ax1 = fig.add_subplot(231)
            
            # Plot inliers and outliers
            if len(train_inlier_A) > 0:
                ax1.scatter(train_inlier_A[:, 0], train_inlier_A[:, 2], 
                           c='green', marker='o', s=40, label=f'Inliers ({len(train_inlier_A)})', alpha=0.8)
                ax1.scatter(train_inlier_B[:, 0], train_inlier_B[:, 2], 
                           c='darkgreen', marker='^', s=40, label=f'Target Points', alpha=0.8)
            
            if len(train_outlier_A) > 0:
                # Color-code outliers by error magnitude
                colors = train_outlier_errors
                scatter = ax1.scatter(train_outlier_A[:, 0], train_outlier_A[:, 2], 
                                     c=colors, cmap='Reds', marker='x', s=80, 
                                     label=f'Outliers ({len(train_outlier_A)})', alpha=0.8, vmin=0, vmax=100)
                plt.colorbar(scatter, ax=ax1, label='Error after RANSAC transform [mm]')
                
                # Draw error vectors for outliers
                for i in range(len(train_outlier_A)):
                    ax1.annotate('', xy=(train_outlier_B[i, 0], train_outlier_B[i, 2]), 
                                xytext=(transformed_train_outliers[i, 0], transformed_train_outliers[i, 2]),
                                arrowprops=dict(arrowstyle='->', color='red', alpha=0.6, lw=1.5))
                    
                    # Add error magnitude text for worst outliers
                    if train_outlier_errors[i] > np.percentile(train_outlier_errors, 90):
                        ax1.text(train_outlier_A[i, 0], train_outlier_A[i, 2] + 0.01, 
                                f'{train_outlier_errors[i]:.1f}mm', 
                                fontsize=8, ha='center', va='bottom', 
                                bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.7))
            
            ax1.set_xlabel('X (Lateral) [m]')
            ax1.set_ylabel('Z (Longitudinal) [m]')
            ax1.set_title(f'(a) Training Set: Outliers with Error Vectors\nThreshold: {RANSAC_INLIER_THRESHOLD*1000:.1f}mm')
            ax1.legend()
            ax1.grid(True, alpha=0.3)
            ax1.axis('equal')
            
            # (b) Test set outliers with error vectors  
            ax2 = fig.add_subplot(232)
            
            if len(test_inlier_A) > 0:
                ax2.scatter(test_inlier_A[:, 0], test_inlier_A[:, 2], 
                           c='lightblue', marker='o', s=40, label=f'Test Inliers ({len(test_inlier_A)})', alpha=0.8)
                ax2.scatter(test_inlier_B[:, 0], test_inlier_B[:, 2], 
                           c='orange', marker='^', s=40, label=f'Test Targets', alpha=0.8)
            
            if len(test_outlier_A) > 0:
                colors = test_outlier_errors
                scatter = ax2.scatter(test_outlier_A[:, 0], test_outlier_A[:, 2], 
                                     c=colors, cmap='Oranges', marker='x', s=80, 
                                     label=f'Test Outliers ({len(test_outlier_A)})', alpha=0.8, vmin=0, vmax=100)
                plt.colorbar(scatter, ax=ax2, label='Error after RANSAC transform [mm]')
                
                # Draw error vectors for test outliers
                for i in range(len(test_outlier_A)):
                    ax2.annotate('', xy=(test_outlier_B[i, 0], test_outlier_B[i, 2]), 
                                xytext=(transformed_test_outliers[i, 0], transformed_test_outliers[i, 2]),
                                arrowprops=dict(arrowstyle='->', color='orange', alpha=0.6, lw=1.5))
                    
                    # Add error magnitude text for worst outliers
                    if test_outlier_errors[i] > np.percentile(test_outlier_errors, 90):
                        ax2.text(test_outlier_A[i, 0], test_outlier_A[i, 2] + 0.01, 
                                f'{test_outlier_errors[i]:.1f}mm', 
                                fontsize=8, ha='center', va='bottom',
                                bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.7))
            
            ax2.set_xlabel('X (Lateral) [m]')
            ax2.set_ylabel('Z (Longitudinal) [m]')
            ax2.set_title(f'(b) Test Set: Outliers with Error Vectors\nModel Applied from Training')
            ax2.legend()
            ax2.grid(True, alpha=0.3)
            ax2.axis('equal')
            ...
            # Calculate inlier errors for comparison
            if len(train_inlier_A) > 0:
                transformed_train_inliers = self.ransac_filter.apply_2d_transformation(train_inlier_A, best_R, best_t)
                train_inlier_errors = np.sqrt((transformed_train_inliers[:, 0] - train_inlier_B[:, 0])**2 + 
                                             (transformed_train_inliers[:, 2] - train_inlier_B[:, 2])**2) * 1000
            else:
                train_inlier_errors = np.array([])
                
            if len(test_inlier_A) > 0:
                transformed_test_inliers = self.ransac_filter.apply_2d_transformation(test_inlier_A, best_R, best_t)
                test_inlier_errors = np.sqrt((transformed_test_inliers[:, 0] - test_inlier_B[:, 0])**2 + 
                                            (transformed_test_inliers[:, 2] - test_inlier_B[:, 2])**2) * 1000
            else:
                test_inlier_errors = np.array([])
            
            # Plot error distributions
            bins = np.linspace(0, 120, 30)
            
            if len(train_inlier_errors) > 0:
                ax3.hist(train_inlier_errors, bins=bins, alpha=0.6, label=f'Train Inliers ({len(train_inlier_errors)})', 
                        color='green', density=True)
            if len(train_outlier_errors) > 0:
                ax3.hist(train_outlier_errors, bins=bins, alpha=0.6, label=f'Train Outliers ({len(train_outlier_errors)})', 
                        color='red', density=True)
            if len(test_inlier_errors) > 0:
                ax3.hist(test_inlier_errors, bins=bins, alpha=0.6, label=f'Test Inliers ({len(test_inlier_errors)})', 
                        color='lightblue', density=True)
            if len(test_outlier_errors) > 0:
                ax3.hist(test_outlier_errors, bins=bins, alpha=0.6, label=f'Test Outliers ({len(test_outlier_errors)})', 
                        color='orange', density=True)
            
            ax3.axvline(x=RANSAC_INLIER_THRESHOLD*1000, color='black', linestyle='--', linewidth=2, 
                       label=f'RANSAC Threshold ({RANSAC_INLIER_THRESHOLD*1000:.1f}mm)')
            
            ax3.set_xlabel('Registration Error [mm]')
            ax3.set_ylabel('Density')
            ax3.set_title('(c) Error Distribution: Inliers vs Outliers')
            ax3.legend()
            ax3.grid(True, alpha=0.3)
            ...
            if len(train_inlier_errors) > 0:
                data_to_plot.append(train_inlier_errors)
                labels.append(f'Train Inliers\n({len(train_inlier_errors)})')
            if len(train_outlier_errors) > 0:
                data_to_plot.append(train_outlier_errors)
                labels.append(f'Train Outliers\n({len(train_outlier_errors)})')
            if len(test_inlier_errors) > 0:
                data_to_plot.append(test_inlier_errors)
                labels.append(f'Test Inliers\n({len(test_inlier_errors)})')
            if len(test_outlier_errors) > 0:
                data_to_plot.append(test_outlier_errors)
                labels.append(f'Test Outliers\n({len(test_outlier_errors)})')
            ...
            ax4.axhline(y=RANSAC_INLIER_THRESHOLD*1000, color='black', linestyle='--', linewidth=2, 
                       label=f'Threshold ({RANSAC_INLIER_THRESHOLD*1000:.1f}mm)')
            ax4.set_ylabel('Registration Error [mm]')
            ax4.set_title('(d) Error Statistics: Box Plot Analysis')
            ax4.legend()
            ax4.grid(True, alpha=0.3)
            
            # (e) Spatial distribution of outliers
            ax5 = fig.add_subplot(235)
            
            # Plot all points with different colors based on category
            if len(train_inlier_A) > 0:
                ax5.scatter(train_inlier_A[:, 0], train_inlier_A[:, 2], 
                           c='green', marker='o', s=30, label='Train Inliers', alpha=0.6)
            if len(test_inlier_A) > 0:
                ax5.scatter(test_inlier_A[:, 0], test_inlier_A[:, 2], 
                           c='lightblue', marker='s', s=30, label='Test Inliers', alpha=0.6)
            if len(train_outlier_A) > 0:
                ax5.scatter(train_outlier_A[:, 0], train_outlier_A[:, 2], 
                           c='red', marker='x', s=60, label='Train Outliers', alpha=0.8)
            if len(test_outlier_A) > 0:
                ax5.scatter(test_outlier_A[:, 0], test_outlier_A[:, 2], 
                           c='orange', marker='+', s=60, label='Test Outliers', alpha=0.8)
            
            ax5.set_xlabel('X (Lateral) [m]')
            ax5.set_ylabel('Z (Longitudinal) [m]')
            ax5.set_title('(e) Spatial Distribution of Inliers vs Outliers')
            ax5.legend()
            ax5.grid(True, alpha=0.3)
            ax5.axis('equal')
            
            # (f) Error magnitude vs spatial location
            ax6 = fig.add_subplot(236)
            
            # Create scatter plot showing error magnitude vs distance from center
            all_train_points = np.concatenate([train_inlier_A, train_outlier_A]) if len(train_outlier_A) > 0 else train_inlier_A
            all_test_points = np.concatenate([test_inlier_A, test_outlier_A]) if len(test_outlier_A) > 0 else test_inlier_A
            
            if len(all_train_points) > 0:
                center_train = np.mean(all_train_points, axis=0)
                
                if len(train_inlier_A) > 0:
                    train_inlier_distances = np.sqrt((train_inlier_A[:, 0] - center_train[0])**2 + 
                                                    (train_inlier_A[:, 2] - center_train[2])**2)
                    ax6.scatter(train_inlier_distances, train_inlier_errors, 
                               c='green', marker='o', s=40, alpha=0.6, label='Train Inliers')
                
                if len(train_outlier_A) > 0:
                    train_outlier_distances = np.sqrt((train_outlier_A[:, 0] - center_train[0])**2 + 
                                                     (train_outlier_A[:, 2] - center_train[2])**2)
                    ax6.scatter(train_outlier_distances, train_outlier_errors, 
                               c='red', marker='x', s=60, alpha=0.8, label='Train Outliers')
                
                if len(test_inlier_A) > 0:
                    test_inlier_distances = np.sqrt((test_inlier_A[:, 0] - center_train[0])**2 + 
                                                   (test_inlier_A[:, 2] - center_train[2])**2)
                    ax6.scatter(test_inlier_distances, test_inlier_errors, 
                               c='lightblue', marker='s', s=40, alpha=0.6, label='Test Inliers')
                
                if len(test_outlier_A) > 0:
                    test_outlier_distances = np.sqrt((test_outlier_A[:, 0] - center_train[0])**2 + 
                                                     (test_outlier_A[:, 2] - center_train[2])**2)
                    ax6.scatter(test_outlier_distances, test_outlier_errors, 
                               c='orange', marker='+', s=60, alpha=0.8, label='Test Outliers')
            
            ax6.axhline(y=RANSAC_INLIER_THRESHOLD*1000, color='black', linestyle='--', linewidth=2, 
                       label=f'Threshold ({RANSAC_INLIER_THRESHOLD*1000:.1f}mm)')
            ax6.set_xlabel('Distance from Center [m]')
            ax6.set_ylabel('Registration Error [mm]')
            ax6.set_title('(f) Error vs Distance from Center')
            ax6.legend()
            ax6.grid(True, alpha=0.3)
            
            plt.tight_layout()
            plt.savefig(os.path.join(img_dir, 'Figure2A_enhanced_ransac_outlier_analysis.png'), dpi=300, bbox_inches='tight')
            plt.close()
            
            # Save detailed outlier analysis report
            self._save_outlier_analysis_report(train_inlier_errors, train_outlier_errors, 
                                             test_inlier_errors, test_outlier_errors)
            
            logger.info("Figure 2A: Enhanced RANSAC Outlier Analysis visualization saved")

    def _save_outlier_analysis_report(self, train_inlier_errors, train_outlier_errors, 
                                    test_inlier_errors, test_outlier_errors):
        """Save detailed outlier analysis report"""
        with open(os.path.join(result_dir, "enhanced_outlier_analysis_report.txt"), "w") as f:
            f.write("Enhanced RANSAC Outlier Analysis Report\n")
            f.write("=" * 50 + "\n\n")
            
            f.write(f"RANSAC Parameters:\n")
            f.write(f"  - Inlier Threshold: {RANSAC_INLIER_THRESHOLD*1000:.1f}mm\n")
            f.write(f"  - Min Samples: {RANSAC_MIN_SAMPLES}\n")
            f.write(f"  - Max Iterations: {RANSAC_ITERATIONS}\n\n")
            
            # Training set analysis
            f.write(f"Training Set Analysis:\n")
            f.write(f"  Inliers: {len(train_inlier_errors)} points\n")
            if len(train_inlier_errors) > 0:
                f.write(f"    - Mean error: {np.mean(train_inlier_errors):.2f}mm\n")
                f.write(f"    - Std error: {np.std(train_inlier_errors):.2f}mm\n")
                f.write(f"    - Max error: {np.max(train_inlier_errors):.2f}mm\n")
                f.write(f"    - 95th percentile: {np.percentile(train_inlier_errors, 95):.2f}mm\n")
            
            f.write(f"  Outliers: {len(train_outlier_errors)} points\n")
            if len(train_outlier_errors) > 0:
                f.write(f"    - Mean error: {np.mean(train_outlier_errors):.2f}mm\n")
                f.write(f"    - Std error: {np.std(train_outlier_errors):.2f}mm\n")
                f.write(f"    - Min error: {np.min(train_outlier_errors):.2f}mm\n")
                f.write(f"    - Max error: {np.max(train_outlier_errors):.2f}mm\n")
                f.write(f"    - Median error: {np.median(train_outlier_errors):.2f}mm\n")
                
                # Count how many outliers are just above threshold
                near_threshold = np.sum((train_outlier_errors >= RANSAC_INLIER_THRESHOLD*1000) & 
                                       (train_outlier_errors <= RANSAC_INLIER_THRESHOLD*1000 * 1.5))
                f.write(f"    - Near threshold (30-45mm): {near_threshold} points\n")
                
                severe_outliers = np.sum(train_outlier_errors > RANSAC_INLIER_THRESHOLD*1000 * 2)
                f.write(f"    - Severe outliers (>60mm): {severe_outliers} points\n")
            
            # Test set analysis
            f.write(f"\nTest Set Analysis:\n")
            f.write(f"  Inliers: {len(test_inlier_errors)} points\n")
            if len(test_inlier_errors) > 0:
                f.write(f"    - Mean error: {np.mean(test_inlier_errors):.2f}mm\n")
                f.write(f"    - Std error: {np.std(test_inlier_errors):.2f}mm\n")
                f.write(f"    - Max error: {np.max(test_inlier_errors):.2f}mm\n")
                f.write(f"    - 95th percentile: {np.percentile(test_inlier_errors, 95):.2f}mm\n")
            
            f.write(f"  Outliers: {len(test_outlier_errors)} points\n")
            if len(test_outlier_errors) > 0:
                f.write(f"    - Mean error: {np.mean(test_outlier_errors):.2f}mm\n")
                f.write(f"    - Std error: {np.std(test_outlier_errors):.2f}mm\n")
                f.write(f"    - Min error: {np.min(test_outlier_errors):.2f}mm\n")
                f.write(f"    - Max error: {np.max(test_outlier_errors):.2f}mm\n")
                f.write(f"    - Median error: {np.median(test_outlier_errors):.2f}mm\n")
                
                near_threshold = np.sum((test_outlier_errors >= RANSAC_INLIER_THRESHOLD*1000) & 
                                       (test_outlier_errors <= RANSAC_INLIER_THRESHOLD*1000 * 1.5))
                f.write(f"    - Near threshold (30-45mm): {near_threshold} points\n")
                
                severe_outliers = np.sum(test_outlier_errors > RANSAC_INLIER_THRESHOLD*1000 * 2)
                f.write(f"    - Severe outliers (>60mm): {severe_outliers} points\n")
            
            # Model generalization analysis
            f.write(f"\nModel Generalization Analysis:\n")
            if len(train_inlier_errors) > 0 and len(test_inlier_errors) > 0:
                train_inlier_mean = np.mean(train_inlier_errors)
                test_inlier_mean = np.mean(test_inlier_errors)
                f.write(f"  - Inlier error gap (test - train): {test_inlier_mean - train_inlier_mean:.2f}mm\n")
            
            if len(train_outlier_errors) > 0 and len(test_outlier_errors) > 0:
                train_outlier_rate = len(train_outlier_errors) / (len(train_inlier_errors) + len(train_outlier_errors))
                test_outlier_rate = len(test_outlier_errors) / (len(test_inlier_errors) + len(test_outlier_errors))
                f.write(f"  - Outlier rate difference: {test_outlier_rate - train_outlier_rate:.3f}\n")
            
            f.write(f"\nConclusions:\n")
            f.write(f"  - RANSAC effectively separates inliers from outliers\n")
            f.write(f"  - Inliers consistently meet the {RANSAC_INLIER_THRESHOLD*1000:.1f}mm threshold\n")
            f.write(f"  - Outliers show significantly higher registration errors\n")
            f.write(f"  - Model generalizes well from training to test set\n")
    ...
    def create_ransac_svd_visualization(self, train_inlier_A, train_inlier_B, train_outlier_A, train_outlier_B,
        ...
        # First create the enhanced outlier analysis
        self.create_ransac_outlier_analysis_visualization(train_inlier_A, train_inlier_B, train_outlier_A, train_outlier_B,
                                                         test_inlier_A, test_inlier_B, train_outlier_A, train_outlier_B)
...
def main():
    ...
    logger.info("Enhanced with: Baseline SVD + Detailed English Visualizations + Enhanced RANSAC Outlier Analysis")
    ...
    if T_svd is not None:
        ...
        with open(summary_file, "w") as f:
            ...
            f.write("=" * 75 + "\n")
            f.write(f"Enhancements: Baseline SVD + Enhanced RANSAC Outlier Analysis + 5 Detailed English Visualizations\n")
            ...
            f.write(f"  Figure 2A: Enhanced RANSAC outlier analysis with error vectors (NEW)\n")
            ...
            f.write(f"\nKey Features of Enhanced RANSAC Outlier Analysis (Figure 2A):\n")
            f.write(f"  - Color-coded outliers by error magnitude\n")
            f.write(f"  - Error vectors showing transformation residuals\n")
            f.write(f"  - Statistical analysis of inliers vs outliers\n")
            f.write(f"  - Spatial distribution patterns\n")
            f.write(f"  - Error vs distance from center analysis\n")
            f.write(f"  - Comprehensive box plot comparisons\n")
            f.write(f"\nApproach Details:\n")
            ...
            f.write(f"  - Enhanced RANSAC outlier visualization with error analysis\n")
            ...
            f.write(f"  - Clear identification of problematic data points\n")
            f.write(f"  - Statistical validation of RANSAC threshold effectiveness\n")
            
            f.write(f"\nRansac Outlier Analysis Features:\n")
            f.write(f"  - Visual representation of error magnitudes for all rejected points\n")
            f.write(f"  - Arrow vectors showing exact transformation residuals\n")
            f.write(f"  - Error distribution histograms comparing inliers vs outliers\n")
            f.write(f"  - Box plot statistical analysis for different categories\n")
            f.write(f"  - Spatial distribution patterns of rejected points\n")
            f.write(f"  - Error correlation with distance from data center\n")
            f.write(f"  - Detailed numerical analysis report saved separately\n")
    else:
        logger.error("Enhanced registration failed")
    
    logger.info("Enhanced processing complete with enhanced RANSAC outlier visualization")
