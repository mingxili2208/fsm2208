...
class RobustRBFModel:
    ...
    def __init__(self, method='thin_plate_spline', smoothing=0.001, constrain_y=True, 
                 adaptive_smoothing=True, min_smoothing=1e-6):
        self.method = method
    ...
    def learn_deformation(self, source_points, residual_vectors):
        ...
        if self.constrain_y:
            ...
            if self.adaptive_smoothing:
                self.actual_smoothing = self._compute_adaptive_smoothing(source_xz, residual_xz)
            else:
    ...
    def _compute_adaptive_smoothing(self, source_points, residual_vectors):
        """计算自适应平滑参数"""
        # 基于数据密度和噪声水平调整平滑参数
        ...
        noise_level = np.std(residual_magnitudes)
        
        # 自适应公式
        base_smoothing = max(self.smoothing, self.min_smoothing)
        
        # 如果点很密集，增加平滑
        if mean_min_distance < 0.01:  # 1cm
            density_factor = 10.0
        elif mean_min_distance < 0.05:  # 5cm
            density_factor = 5.0
        else:
            density_factor = 1.0
        
        # 如果噪声很大，增加平滑
        if noise_level > 0.01:  # 1cm噪声
            noise_factor = 5.0
        elif noise_level > 0.005:  # 5mm噪声
            noise_factor = 2.0
        else:
            noise_factor = 1.0
        
        adaptive_smoothing = base_smoothing * density_factor * noise_factor
        
        logger.info(f"Adaptive smoothing computation:")
        logger.info(f"  Mean min distance: {mean_min_distance*1000:.3f}mm")
        logger.info(f"  Noise level: {noise_level*1000:.3f}mm")
        logger.info(f"  Density factor: {density_factor}")
        logger.info(f"  Noise factor: {noise_factor}")
        logger.info(f"  Final smoothing: {adaptive_smoothing:.6f}")
...
class HyperparameterOptimizer:
    ...
    def optimize_rbf_params(self, positions_A, positions_B, T_svd):
        ...
        logger.info("Note: smoothing=0.0 has been excluded to avoid numerical instability")
        ...
        for kernel, smoothing in param_combinations:
            ...
            try:
                for fold, (train_idx, test_idx) in enumerate(kf.split(indices)):
                    ...
                    # 训练稳健形变模型
                    model = RobustRBFModel(method=kernel, smoothing=smoothing, 
                                         constrain_y=True, adaptive_smoothing=False)
...
# 继续原有的CoordinateTransformer类，但使用新的RobustRBFModel
class CoordinateTransformer:
    ...
    def two_stage_registration_enhanced(self):
        ...
        logger.info("=== Starting Enhanced Two-Stage Registration ===")
        logger.info("Features: Data preprocessing, robust outlier detection, stable RBF optimization")
        ...
        self.visualize_transformation_stage("SVD Coarse Registration (Enhanced)", 
                                           final_clean_A, final_clean_B, svd_transformed, 
                                           svd_rmse_results, residual_vectors)
        
        # Stage 3: Robust hyperparameter optimization and model selection
        logger.info("Stage 3: Robust Hyperparameter Optimization and Model Selection")
        ...
        # 使用最佳模型训练最终模型
        logger.info(f"Training final robust model: {best_model_info}")
        ...
        if best_model_info['type'] == 'rbf':
            self.deformation_model = RobustRBFModel(
                ...
                constrain_y=True,
                adaptive_smoothing=True  # 启用自适应平滑
            )
        else:  # bspline
        ...
        logger.info(f"Final Overall RMSE (enhanced): {final_rmse_results['overall_rmse']*1000:.3f}mm")
        ...
        logger.info(f"Final residual statistics (enhanced):")
        ...
        # 可视化最终结果
        self.visualize_transformation_stage(f"Final Enhanced {best_model_info['type'].upper()}", 
        ...
        self.residual_analyzer.analyze_final_residuals(
            final_clean_A, final_clean_B, final_transformed, 
            f"Final Enhanced {best_model_info['type'].upper()}"
        )
    ...
    def _print_detailed_results(self, svd_performance, final_performance, best_model_info):
        ...
        logger.info("\n=== Detailed Enhanced Performance Analysis ===")
        ...
        logger.info(f"Final (SVD + Enhanced {best_model_info['type'].upper()}) Results:")
        ...
        logger.info(f"Best Model: Enhanced {best_model_info['type'].upper()}")
    ...
    def _save_transformation_data(self):
        ...
        # 保存最佳模型信息
        model_info_file = os.path.join(result_dir, "enhanced_model_info.txt")
        with open(model_info_file, "w") as f:
            f.write(f"Enhanced Model Type: {self.best_model_type}\n")
        ...
        # 保存离群点检测信息
        outlier_info_file = os.path.join(result_dir, "enhanced_outlier_detection_info.txt")
        with open(outlier_info_file, "w") as f:
            f.write("Enhanced Outlier Detection Summary\n")
            f.write("=" * 40 + "\n")
        ...
        logger.info("Enhanced transformation data and all analysis info saved")
...
def main():
    ...
    logger.info("  - Stable RBF optimization (NO smoothing=0.0)")
    logger.info("  - Adaptive smoothing parameters")
    ...
    # Perform enhanced two-stage registration
    ...
    if T_svd is not None and final_rmse_results is not None:
        ...
        with open(summary_file, "w") as f:
            f.write("Enhanced Two-Stage Registration Summary\n")
            f.write("=" * 80 + "\n")
            ...
            f.write("- Stable RBF optimization (smoothing >= 1e-6)\n")
            f.write("- Adaptive smoothing parameters\n")
