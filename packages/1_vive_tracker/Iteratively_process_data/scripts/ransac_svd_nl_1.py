...
# RANSAC 参数
RANSAC_MIN_SAMPLES = 12  # 最少点数
RANSAC_ITERATIONS = 300  # 迭代次数
RANSAC_INLIER_THRESHOLD = 0.030  # 内点阈值 30mm (0.030m)
RANSAC_MIN_DISTANCE = 1.0  # 最小欧式距离 1m
...
#########################################################
LASER_TRACKER_CSV = os.path.join(parent_dir, "data/crossed_corrected_tracker_data_0707_125849.csv")
...
class RANSACFilter:
    ...
    """
    def __init__(self, min_samples=12, max_iterations=100, inlier_threshold=0.030, min_distance=1.0):
    ...
    def calculate_euclidean_distance(self, points_A, points_B):
        """计算点对之间的欧式距离"""
    ...
    def filter_by_distance(self, positions_A, positions_B):
        """根据欧式距离过滤点对"""
        distances = self.calculate_euclidean_distance(positions_A, positions_B)
        valid_indices = distances >= self.min_distance
        
        filtered_A = np.array(positions_A)[valid_indices]
        filtered_B = np.array(positions_B)[valid_indices]
        
        logger.info(f"Distance filtering: {len(positions_A)} -> {len(filtered_A)} points (removed {np.sum(~valid_indices)} points with distance < {self.min_distance}m)")
        
        return filtered_A, filtered_B, valid_indices
    ...
    def ransac_filter(self, positions_A, positions_B):
        ...
        # 首先根据距离过滤
        positions_A, positions_B, distance_valid = self.filter_by_distance(positions_A, positions_B)
        
        if len(positions_A) < self.min_samples:
            logger.error(f"After distance filtering, only {len(positions_A)} points remain, less than minimum required {self.min_samples}")
    ...
    def save_ransac_results(self, original_A, original_B, inlier_A, inlier_B, outlier_A, outlier_B):
        """保存RANSAC结果"""
        # 保存内点
        if len(inlier_A) > 0:
        ...
        with open(os.path.join(result_dir, "ransac_summary.txt"), "w") as f:
            ...
            f.write(f"  - Min distance: {self.min_distance}m\n")
            f.write(f"\nResults:\n")
...
class CoordinateTransformer:
    ...
    def three_stage_registration(self):
        ...
        self.ransac_filter = RANSACFilter(
            ...
            min_distance=RANSAC_MIN_DISTANCE
        )
...
def main():
    ...
    if T_svd is not None and final_rmse_results is not None:
        ...
        with open(summary_file, "w") as f:
            ...
            f.write(f"  Min distance: {RANSAC_MIN_DISTANCE}m\n")
