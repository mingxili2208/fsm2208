...
class CoordinateTransformer:
    ...
    def two_stage_registration(self):
        """Perform two-stage registration: SVD + ICP (without point filtering)"""
        if self.__positions_A is None or self.__positions_B is None:
            logger.error("Please provide position data first!")
            return None, None
        
        logger.info("=== Starting Two-Stage Registration (No Point Filtering) ===")
        ...
        # Stage 2: ICP fine registration (using all points from SVD)
        logger.info("Stage 2: ICP Fine Registration (using all SVD-transformed points)")
        
        # Use all SVD-transformed points as input for ICP
        logger.info(f"Using all {len(svd_transformed)} points for ICP registration")
        
        # Perform ICP on SVD-transformed points vs target points
        self.T_icp, icp_transformed, rmse_history = self.icp.register(svd_transformed, self.__positions_B)
...
def main():
    """Main function"""
    logger.info("Starting 1:32 scale model tracker data processing with SVD + ICP (no filtering)")
    ...
    if T_total is not None and final_rmse_results is not None:
        ...
        with open(summary_file, "w") as f:
            f.write("Two-Stage Registration Summary (No Point Filtering)\n")
            f.write("=" * 60 + "\n")
            ...
            f.write("\nNote: All points from SVD stage were used in ICP registration\n")
    else:
