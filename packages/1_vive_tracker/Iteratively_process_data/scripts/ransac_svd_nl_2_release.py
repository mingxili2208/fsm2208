...
#########################################################
LASER_TRACKER_CSV = os.path.join(parent_dir, "data/combined_corrected_tracker_data_3.csv")
csv_filename = os.path.basename(LASER_TRACKER_CSV)
output_dir = os.path.join(result_dir, f"RANSAC_SVD_Deformation_{timestamp}_1to32_scale_RMSE_{TARGET_RMSE*1000:.1f}mm_{csv_filename}")
...
class CoordinateTransformer:
    ...
    def calculate_svd_transformation(self, positions_A=None, positions_B=None):
        """Stage 2: SVD-based coarse registration with Y-axis constraint (after RANSAC filtering)"""
        if positions_A is not None and positions_B is not None:
            self.__filtered_positions_A = positions_A
            self.__filtered_positions_B = positions_B
        elif self.__filtered_positions_A is None or self.__filtered_positions_B is None:
            logger.error("Please provide filtered position data first!")
            return

        positions_A = np.array(self.__filtered_positions_A)
        positions_B = np.array(self.__filtered_positions_B)
        ...
        logger.info("SVD transformation matrix calculated (Y-axis constrained, post-RANSAC)")
    ...
    def three_stage_registration(self):
        """Perform three-stage registration: RANSAC + SVD + Non-rigid deformation learning"""
        if self.__positions_A is None or self.__positions_B is None:
            logger.error("Please provide position data first!")
            return None, None
        
        logger.info("=== Starting Three-Stage Registration (RANSAC + SVD + Non-rigid Deformation) ===")
        
        original_positions_A = np.array(self.__positions_A)
        original_positions_B = np.array(self.__positions_B)
        
        # Stage 1: RANSAC data filtering (without distance pre-filtering)
        logger.info("Stage 1: RANSAC Data Filtering (No distance pre-filtering)")
        ...
        # Stage 2: SVD coarse registration
        logger.info("Stage 2: SVD Coarse Registration (Post-RANSAC)")
        self.calculate_svd_transformation()
        
        # Apply SVD transformation
        svd_transformed = self.apply_transformation(self.__filtered_positions_A, self.T_svd)
        
        # Evaluate SVD results
        svd_rmse_results = self.calculate_directional_rmse(self.__filtered_positions_B, svd_transformed)
        svd_performance = self.evaluate_performance(self.__filtered_positions_B, svd_transformed)
        
        logger.info(f"SVD Overall RMSE (post-RANSAC): {svd_rmse_results['overall_rmse']*1000:.3f}mm")
        
        # 计算残差向量
        residual_vectors = self.__filtered_positions_B - svd_transformed
        ...
        self.visualize_transformation_stage("SVD Coarse Registration (Post-RANSAC)", 
                                           self.__filtered_positions_A, self.__filtered_positions_B, svd_transformed, 
                                           svd_rmse_results, residual_vectors)
        
        # Stage 3: Learn non-rigid deformation
        logger.info("Stage 3: Learning Non-rigid Deformation")
        ...
        final_rmse_results = self.calculate_directional_rmse(self.__filtered_positions_B, final_transformed)
        final_performance = self.evaluate_performance(self.__filtered_positions_B, final_transformed)
        
        logger.info(f"Final Overall RMSE: {final_rmse_results['overall_rmse']*1000:.3f}mm")
        
        # 计算形变校正后的残差
        final_residuals = self.__filtered_positions_B - final_transformed
        ...
        self.visualize_transformation_stage("Non-rigid Deformation Final", 
                                           self.__filtered_positions_A, self.__filtered_positions_B, final_transformed, 
                                           final_rmse_results, final_residuals)
        
        # 可视化形变场
        if len(self.__filtered_positions_A) > 0:
            # 计算点云边界
        ...
        self._print_detailed_results(svd_performance, final_performance, len(original_positions_A), len(inlier_A))
    ...
    def _print_detailed_results(self, svd_performance, final_performance, original_count, filtered_count):
        """Print detailed performance analysis"""
        logger.info("\n=== Detailed Performance Analysis ===")
        
        logger.info("RANSAC Filtering Results:")
        logger.info(f"  Original points: {original_count}")
        logger.info(f"  Filtered points: {filtered_count}")
        logger.info(f"  Points removed: {original_count - filtered_count} ({(original_count - filtered_count)/original_count*100:.1f}%)")
        
        logger.info("SVD Stage Results (Post-RANSAC):")
        ...
        logger.info("Final (RANSAC + SVD + Deformation) Results:")
...
def main():
    """Main function"""
    logger.info("Starting 1:32 scale model tracker data processing with RANSAC + SVD + Non-rigid Deformation")
    ...
    logger.info("RANSAC: No distance pre-filtering applied")
    ...
    T_svd, final_rmse_results = transformer.three_stage_registration()
    ...
    if T_svd is not None and final_rmse_results is not None:
        ...
        with open(summary_file, "w") as f:
            f.write("Three-Stage Registration Summary (RANSAC + SVD + Non-rigid Deformation)\n")
            f.write("=" * 80 + "\n")
            ...
            f.write(f"Points after RANSAC: {len(transformer._CoordinateTransformer__filtered_positions_A) if hasattr(transformer, '_CoordinateTransformer__filtered_positions_A') and transformer._CoordinateTransformer__filtered_positions_A is not None else 'N/A'}\n")
            ...
            f.write(f"  Distance pre-filtering: Disabled\n")
            f.write("\nMethod: RANSAC outlier removal + SVD coarse registration + RBF-based non-rigid deformation learning\n")
            f.write("Deformation model: Thin-plate spline interpolation with Y-axis constraint\n")
            f.write("\nOutput files:\n")
            f.write("  - ransac_inlier_data.csv: Filtered point pairs for further processing\n")
            f.write("  - ransac_inliers.txt: Inlier coordinates\n")
            f.write("  - ransac_outliers.txt: Outlier coordinates\n")
    else:
