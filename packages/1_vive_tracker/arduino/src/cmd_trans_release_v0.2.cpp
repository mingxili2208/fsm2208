...
void setup() {
  ...
  // sendStateMessage(0x00,control_command.steering_tire_angle,
  // control_command.speed);
  if (!radio.begin()) {
    sendStateMessage(0x00, control_command.steering_tire_angle,
                     control_command.speed);
  ...
  radio.setChannel(0);      // channel 0 as 2.4GHZ-----（发射频率=2400MHZ +
                             // n_channel）
...
void loop() {
  ...
  if (radio.available()) {
    ...
    //   }
    // }
    if (temp_buf[0] == 0x55 && temp_buf[1] == 0x7e && temp_buf[8] == 0x7e &&
        temp_buf[9] == 0x55) {
      if (temp_buf[2] == 0x03) {
        uint8_t checksum =
            temp_buf[3] ^ temp_buf[4] ^ temp_buf[5] ^ temp_buf[6];
        if (temp_buf[7] == checksum) { // 校验和正确
          // 解析数据
    ...
    if (Serial.available() > 0) {
      ...
      if (data[0] == 0x42) {
        ...
        // data anlysis succeed!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        sendStateMessage(0x01, control_command.steering_tire_angle,
                         control_command.speed);
        ...
        if (ok) {
          // send succeed!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          sendStateMessage(0x02, control_command.steering_tire_angle,
                           control_command.speed);
        } else {
          // send error---------------
          sendStateMessage(0x03, control_command.steering_tire_angle,
                           control_command.speed);
        }
      } else {
        // data receive but analysis error-----------
        sendStateMessage(0x04, control_command.steering_tire_angle,
                         control_command.speed);
      }
    } else {
      // data receive error
      sendStateMessage(0x05, control_command.steering_tire_angle,
                       control_command.speed);
    }
    delay(20);
  }
