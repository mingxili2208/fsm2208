...
class NoiseFilter:
    ...
    def get_status_info(self):
        ...
        if self.baseline_value is not None:
            ...
            return f"Calibrating: {self.baseline_samples}/{self.baseline_required} samples"
...
def resume_recording():
    ...
    if paused:
        ...
        if all_baselines_ready:
            ...
            baselines_status = []
            for name, filter_obj in noise_filters.items():
                baselines_status.append(f"{name}: {filter_obj.get_status_info()}")
...
def on_press(key):
    ...
    try:
        ...
        if key.char == 'r' and current_state == SystemState.IDLE:
            ...
            if current_state == SystemState.RECORDING:
                ...
                if resume_recording():
                    ...
                    operation_successful = True  # 这也算是一个有效的操作响应
        ...
        if not operation_successful and key.char in ['r', 'e', 't', 'p']:
            ...
            elif key.char == 'e' and initial_state not in [SystemState.RECORDING, SystemState.PAUSED]:
            ...
            elif key.char == 'p' and initial_state not in [SystemState.RECORDING, SystemState.PAUSED]:
