...
def create_data_directory():
    ...
    # data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", timestamp)
...
def resume_recording():
    ...
    if paused:
        ...
        if all_baselines_ready:
            ...
            baselines_status = []
            for name, filter_obj in noise_filters.items():
                status = "就绪" if filter_obj.is_baseline_ready() else f"校准中({filter_obj.baseline_samples}/{filter_obj.baseline_required})"
...
采样流程说明:
  1. 按'R'开始录制，进行线性移动
  2. 按'P'暂停录制，可切换到新位置
  3. 在新位置再按'P'，系统会重新校准baseline后继续采样
  4. 最终保存原始数据和滤波数据到两个独立的CSV文件
=========================
    """
...
=== 性能分析报告 ===
系统状态: {current_state}
采样段数: {data_status.current_segment}
暂停次数: {data_status.pause_count}
...
数据统计:
  原始数据点: {len(data_status.recorded_data_raw)}
  滤波数据点: {len(data_status.recorded_data_filtered)}
    """
    write_log(report, "IMPORTANT")
    return report
