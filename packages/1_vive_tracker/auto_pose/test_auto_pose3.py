...
class DataStatus:
    ...
    def get_buffer_average(self):
        ...
        # sum(item[0] for item in self.data_buffer) / len(self.data_buffer)
    ...
    def record_buffer_data(self):
        """记录缓冲区中的所有数据"""
        if self.data_buffer:
            write_log(f"[缓冲] 记录缓冲区数据 ({len(self.data_buffer)}个点)", "IMPORTANT")
            for item in self.data_buffer:
                self.record_data_point(*item)
            self.clear_buffer()
    ...
    def finalize_recording(self):
        """完成录制，处理剩余数据"""
        if self.data_buffer:
            write_log(f"[系统] 停止录制，记录缓冲区中的所有数据 ({len(self.data_buffer)}个点)", "IMPORTANT")
            self.record_buffer_data()
...
def process_data_with_buffer(timestamp, distance_2, distance_3, cam_coord, recovery_needed=False):
    ...
    if significant_changes:
        ...
        # 记录缓冲区数据
        data_status.record_buffer_data()
        ...
        if len(data_status.data_buffer) >= buffer_size:
            # 计算平均值并记录
            avg_data = data_status.get_buffer_average()
            if avg_data:
                write_log(f"[缓冲] 缓冲区满，记录平均值数据", "IMPORTANT")
                data_status.record_data_point(*avg_data)
                data_status.last_recorded_data = (avg_data[1], avg_data[2], avg_data[3][0], avg_data[3][2])
                data_status.clear_buffer()
