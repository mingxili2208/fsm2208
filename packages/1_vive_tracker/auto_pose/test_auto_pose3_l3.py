...
class LaserDataCollector:
    ...
    def get_stable_laser_data(self):
        """连续请求3次激光数据，如果误差小于阈值则返回平均值"""
        laser_readings = []
        
        for i in range(3):
            self.send_a69_data_request()
            # 尽可能快地读取，减少延迟
            time.sleep(0.005)  # 5ms延迟，确保数据到达
            
            try:
                response = self.ser.read(10)
                parsed_data = self.parse_a69_data(response)
                
                if parsed_data:
                    laser_readings.append(parsed_data)
                    write_log(f"[激光] 第{i+1}次读取: d2={parsed_data[0]:.3f}m, d3={parsed_data[1]:.3f}m")
                else:
                    write_log(f"[激光] 第{i+1}次读取失败")
                    return None
                    
            except serial.SerialException as e:
                write_log(f"[错误] 激光数据读取失败: {e}", "ERROR")
                return None
        
        # 检查3次读取的误差
        if len(laser_readings) == 3:
            # 计算distance_2和distance_3的误差
            d2_values = [reading[0] for reading in laser_readings]
            d3_values = [reading[1] for reading in laser_readings]
            
            d2_max_error = max(d2_values) - min(d2_values)
            d3_max_error = max(d3_values) - min(d3_values)
            
            write_log(f"[激光] 误差检查: d2误差={d2_max_error:.4f}, d3误差={d3_max_error:.4f}, 阈值={self.laser_error_threshold}")
            
            if d2_max_error <= self.laser_error_threshold and d3_max_error <= self.laser_error_threshold:
                # 误差在阈值内，返回平均值
                avg_d2 = sum(d2_values) / 3
                avg_d3 = sum(d3_values) / 3
                write_log(f"[激光] 误差符合要求，使用平均值: d2={avg_d2:.3f}m, d3={avg_d3:.3f}m", "IMPORTANT")
                return (avg_d2, avg_d3)
            else:
                write_log(f"[激光] 误差过大，丢弃本次数据", "IMPORTANT")
                return None
...
# 初始化激光数据采集器
laser_collector = LaserDataCollector(ser)
...
def serial_listener():
    ...
    while running:
        if recording:
            # 获取稳定的激光数据（连续3次，误差小于阈值）
            laser_data = laser_collector.get_stable_laser_data()
            ...
            if laser_data:
                ...
                try:
                    ...
                    write_log(f"[Tracker] 获取位姿数据: x={cam_coord[0]:.3f}, y={cam_coord[1]:.3f}, z={cam_coord[2]:.3f}")
                    ...
                    write_log(f"[错误] Tracker数据获取失败: {e}", "ERROR")
                ...
                if consecutive_failures >= max_consecutive_failures:
                    write_log(f"[警告] 连续 {consecutive_failures} 次未收到稳定的激光数据，请检查连接", "ERROR")
                write_log("[警告] 未收到稳定的激光数据")
...
def on_press(key):
    ...
    try:
        if key.char == 'r' and not recording:
            ...
            write_log(f"[系统] 激光数据误差阈值: {laser_collector.laser_error_threshold}", "IMPORTANT")
