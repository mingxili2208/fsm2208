...
1. BASELINE CALIBRATION:
   - Each filter requires 50 samples to establish baseline
   - Baseline automatically adapts during stable periods
   - Baseline resets when 'P' is pressed (position change)

2. SAMPLING SEGMENTS:
   - Segment 1: Initial recording after pressing 'R'
   - Segment N: Each time 'P' is pressed, increment segment number
...
3. DATA RECORDING CONDITIONS:
   ...
   - Significant change detection based on thresholds:
     * Distance_2: >0.008m change
     * Distance_3: >0.008m change  
     * X coordinate: >0.005m change
     * Z coordinate: >0.005m change
   - Maximum change protection: reject if any parameter >0.2m change
...
5. OUTPUT FILES:
   ...
   - Filtered data: after noise filtering applied
   - Both files include segment numbers for analysis
"""
...
class NoiseFilter:
    ...
    def start_baseline_update(self):
        """开始基线更新过程"""
        self.data_history.clear()
        self.baseline_value = None
        self.baseline_samples = 0
        self.baseline_updating = True
        write_log(f"[滤波] 开始基线校准...", "IMPORTANT")
    ...
    def add_sample(self, value):
        ...
        if self.baseline_updating and self.baseline_samples < self.baseline_required:
    ...
    def is_baseline_ready(self):
        """检查基线是否准备就绪"""
        return self.baseline_value is not None and not self.baseline_updating
    
    def is_baseline_updating(self):
        """检查基线是否正在更新中"""
        return self.baseline_updating
    
    def get_status_info(self):
        """获取详细状态信息"""
        if self.baseline_updating:
            return f"Updating: {self.baseline_samples}/{self.baseline_required} samples"
        elif self.baseline_value is not None:
            return f"Baseline: {self.baseline_value:.4f}m ({self.baseline_samples} samples)"
        else:
            return f"Not calibrated"
...
class DataStatus:
    def __init__(self):
        ...
        self.pause_count = 0             # 暂停次数计数（按P的次数）
    ...
    def increment_segment_on_pause(self):
        """在暂停时增加采样段数"""
        self.pause_count += 1
        # 只有在不是第一次暂停时才增加段数（第一段已经存在）
        if self.pause_count > 0:
            self.current_segment = self.pause_count + 1
        self.last_recorded_data = None
        write_log(f"[采样] 切换到采样段 #{self.current_segment} (总暂停次数: {self.pause_count})", "IMPORTANT")
...
def get_current_state():
    ...
    elif any(filter_obj.is_baseline_updating() for filter_obj in noise_filters.values()):
        return SystemState.WAITING_BASELINE
    elif recording:
...
def pause_recording():
    ...
    if recording and not paused:
        ...
        # 重置滤波器的baseline（但不开始更新）
        for name, filter_obj in noise_filters.items():
            filter_obj.reset_baseline()
        
        write_log(f"[采样] 已暂停录制，请移动到新位置后按'P'恢复采样", "IMPORTANT")
        write_log(f"[系统] 系统状态: {current_state}", "IMPORTANT")
        return True
    return False
...
def resume_recording():
    ...
    if paused:
        ...
        # 开始baseline更新过程
        for name, filter_obj in noise_filters.items():
            filter_obj.start_baseline_update()
        
        current_state = SystemState.WAITING_BASELINE
        write_log(f"[采样] 恢复采样，开始基线校准...", "IMPORTANT")
        write_log(f"[系统] 系统状态: {current_state}", "IMPORTANT")
        return True
    return False
...
def simplified_process_data(timestamp, distance_2, distance_3, cam_coord, recovery_needed=False):
    global data_status, noise_filters, drift_validators, paused, current_state
    ...
    # 检查baseline校准状态并更新系统状态
    if current_state == SystemState.WAITING_BASELINE:
        ...
        if all_baselines_ready:
            ...
            write_log(f"[采样] 基线校准完成，开始正常录制", "IMPORTANT")
            write_log(f"[系统] 系统状态: {current_state}", "IMPORTANT")
    ...
    # 如果处于等待baseline状态，只用于baseline校准，不记录数据
    if current_state == SystemState.WAITING_BASELINE:
        return
...
def enhanced_serial_listener():
    ...
    while running:
        if recording:  # 录制模式包括正常录制和等待baseline状态
...
Filter Statistics:
  D2: {noise_filters['d2'].get_status_info()}
  D3: {noise_filters['d3'].get_status_info()}
Data Statistics:
...
def on_press(key):
    ...
    try:
        ...
        if key.char == 'r' and current_state == SystemState.IDLE:
        ...
        elif key.char == 'e' and (current_state in [SystemState.RECORDING, SystemState.PAUSED, SystemState.WAITING_BASELINE]):
            recording = False
            paused = False
            current_state = SystemState.IDLE
        ...
        elif key.char == 'p' and (current_state in [SystemState.RECORDING, SystemState.PAUSED, SystemState.WAITING_BASELINE]):
            if current_state == SystemState.RECORDING:
                ...
                # 恢复录制并开始baseline校准
                if resume_recording():
                    write_log("[采样] 开始恢复采样并校准基线", "IMPORTANT")
                    operation_successful = True
            elif current_state == SystemState.WAITING_BASELINE:
                write_log("[采样] 基线校准进行中，请等待完成", "IMPORTANT")
                operation_successful = True
            ...
            # 显示测试状态信息
            write_log(f"Current System State: {current_state}", "IMPORTANT")
            write_log(f"Sampling Segment: {data_status.current_segment}, Pause Count: {data_status.pause_count}", "IMPORTANT")
            
            # 显示baseline状态
            for name, filter_obj in noise_filters.items():
                write_log(f"Filter {name}: {filter_obj.get_status_info()}", "IMPORTANT")
            
            for name, validator in drift_validators.items():
                if validator.is_testing:
                    status = validator.get_test_status()
                    write_log(f"Test {name}: {status}", "IMPORTANT")
            if not any(v.is_testing for v in drift_validators.values()):
                write_log("No active static tests", "IMPORTANT")
            operation_successful = True
        ...
        if not operation_successful and key.char in ['r', 'e', 't', 'p']:
            if key.char == 'r' and initial_state != SystemState.IDLE:
                write_log(f"Warning: Current state is {initial_state}, cannot start recording. Press 'X' to exit current mode first", "IMPORTANT")
            elif key.char == 'e' and initial_state not in [SystemState.RECORDING, SystemState.PAUSED, SystemState.WAITING_BASELINE]:
                write_log(f"Warning: Current state is {initial_state}, cannot stop recording", "IMPORTANT")
            elif key.char == 't' and initial_state != SystemState.IDLE:
                write_log(f"Warning: Current state is {initial_state}, cannot start test. Press 'X' to exit current mode first", "IMPORTANT")
            elif key.char == 'p' and initial_state not in [SystemState.RECORDING, SystemState.PAUSED, SystemState.WAITING_BASELINE]:
                write_log(f"Warning: Current state is {initial_state}, cannot pause/resume recording", "IMPORTANT")
