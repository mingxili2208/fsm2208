...
class IndustrialDataProcessor:
    ...
    def synchronize_data(self, preprocessed_lidar_data):
        ...
        for lidar_point in lidar_subset:
            ...
            self.interpolation_info.append({
                ...
                'alpha': alpha if interpolation_type == "linear" else 0,
    ...
    def generate_coordinate_comparison_plot(self, final_coordinates):
        ...
        # 1. 散点图对比
        axes[0, 0].scatter(world_x, world_z, c='red', alpha=0.6, s=20, label='World Coordinates')
        axes[0, 0].scatter(tracker_x, tracker_z, c='blue', alpha=0.6, s=20, label='Tracker Coordinates')
        ...
        axes[0, 1].plot(timestamps, world_x, 'r-', linewidth=2, label='World X', alpha=0.8)
        axes[0, 1].plot(timestamps, tracker_x, 'b-', linewidth=2, label='Tracker X', alpha=0.8)
        ...
        # 3. Z坐标时序对比
        axes[1, 0].plot(timestamps, world_z, 'r-', linewidth=2, label='World Z', alpha=0.8)
        axes[1, 0].plot(timestamps, tracker_z, 'b-', linewidth=2, label='Tracker Z', alpha=0.8)
        ...
        axes[1, 1].scatter(diff_x, diff_z, c='green', alpha=0.6, s=20)
        ...
        axes[1, 1].set_title('Coordinate Differences')
    ...
    def generate_sg_filtering_effect_plot(self):
        ...
        for i in range(max_segments):
            ...
            # Distance 2 比较
            axes[i, 0].plot(data['timestamps'], data['d2_raw'] * 1000, 'b-', 
                           linewidth=1, alpha=0.7, label='Raw Data')
            axes[i, 0].plot(data['timestamps'], data['d2_filtered'] * 1000, 'r-', 
                           linewidth=2, label='SG Filtered')
            axes[i, 0].set_title(f'Segment {data["segment_id"]} - Distance 2 (Window={data["window_length"]}, Poly={data["polyorder"]})')
            axes[i, 0].set_ylabel('Distance (mm)')
            axes[i, 0].legend()
            axes[i, 0].grid(True, alpha=0.3)
            
            # Distance 3 比较
            axes[i, 1].plot(data['timestamps'], data['d3_raw'] * 1000, 'b-', 
                           linewidth=1, alpha=0.7, label='Raw Data')
            axes[i, 1].plot(data['timestamps'], data['d3_filtered'] * 1000, 'r-', 
                           linewidth=2, label='SG Filtered')
            axes[i, 1].set_title(f'Segment {data["segment_id"]} - Distance 3 (Window={data["window_length"]}, Poly={data["polyorder"]})')
    ...
    def generate_interpolation_effect_plot(self, synchronized_data):
        ...
        axes[0, 0].pie(interp_types.values, labels=interp_types.index, autopct='%1.1f%%', 
                      colors=colors[:len(interp_types)])
        axes[0, 0].set_title('Interpolation Type Distribution')
        
        # 2. 时间差分布
        axes[0, 1].hist(interp_df['time_diff'] * 1000, bins=30, alpha=0.7, color='green', edgecolor='black')
        ...
        axes[0, 1].axvline(x=mean_diff, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_diff:.1f}ms')
        axes[0, 1].legend()
        
        # 3. Alpha值分布（线性插值权重）
        linear_data = interp_df[interp_df['interpolation_type'] == 'linear']
        if len(linear_data) > 0:
            axes[1, 0].hist(linear_data['alpha'], bins=20, alpha=0.7, color='orange', edgecolor='black')
        ...
        axes[1, 1].plot(tracker_times, self.tracker_data['x'], 'k-', alpha=0.3, linewidth=1, label='Tracker Raw Data')
        
        # 标记插值点
        if np.any(linear_mask):
            linear_times = interp_df[linear_mask]['time_norm']
            linear_x = sync_df[sync_df['timestamp'].isin(interp_df[linear_mask]['lidar_timestamp'])]['x']
            axes[1, 1].scatter(linear_times, linear_x, c='red', s=30, alpha=0.8, label='Linear Interpolated', zorder=5)
        
        if np.any(nearest_mask):
            nearest_times = interp_df[nearest_mask]['time_norm']
            nearest_x = sync_df[sync_df['timestamp'].isin(interp_df[nearest_mask]['lidar_timestamp'])]['x']
            axes[1, 1].scatter(nearest_times, nearest_x, c='blue', s=30, alpha=0.8, label='Nearest Neighbor', zorder=5)
