...
class IndustrialDataProcessor:
    ...
    def filter_by_pose_stability(self, synchronized_data):
        """
        Step 3: Global point pair filtering based on Tracker pose stability.
        New Logic: Filter using the mean ± threshold of the most populated bin for each angle.
        This version now also calls a plotting function to visualize the filtering effect.
        """
        if not self.config["pose_filtering"]["enable_pose_stability_filter"]:
            self.logger.info("Pose stability filtering disabled, skipping")
            return synchronized_data
        
        self.logger.info("Starting pose stability filtering with new logic (mean of dominant bin ± threshold)...")
        
        if len(synchronized_data) == 0:
            self.logger.warning("No synchronized data for pose filtering")
            return synchronized_data
        
        # Extract angle data and convert to degrees
        angles = {
            'roll': np.array([p['roll'] for p in synchronized_data]),
            'pitch': np.array([p['pitch'] for p in synchronized_data]),
            'yaw': np.array([p['yaw'] for p in synchronized_data])
        ...
        filter_masks = {}
        plot_info = {} # Dictionary to store data for visualization

        for angle_name, angle_data in angles.items():
            self.logger.info(f"Analyzing {angle_name.capitalize()} angle...")
            
            hist, bin_edges = np.histogram(angle_data, bins=bins)
            max_count_idx = np.argmax(hist)
            
            dominant_bin_start = bin_edges[max_count_idx]
            dominant_bin_end = bin_edges[max_count_idx + 1]
            
            points_in_dominant_bin_mask = (angle_data >= dominant_bin_start) & (angle_data <= dominant_bin_end)
            points_in_dominant_bin = angle_data[points_in_dominant_bin_mask]
            
            if len(points_in_dominant_bin) == 0:
                self.logger.warning(f"  - Dominant bin for {angle_name} is empty. Skipping filtering for this axis.")
                filter_masks[angle_name] = np.ones(len(angle_data), dtype=bool)
                # Store info for plotting even if skipped
                plot_info[angle_name] = {'original_data': angle_data, 'skipped': True}
                continue

            dominant_mean = np.mean(points_in_dominant_bin)
            threshold = thresholds[angle_name]
            filter_lower_bound = dominant_mean - threshold
            filter_upper_bound = dominant_mean + threshold
            
            self.logger.info(f"  - Dominant bin mean: {dominant_mean:.2f}°. Applying filter range: [{filter_lower_bound:.2f}°, {filter_upper_bound:.2f}°]")

            final_mask = (angle_data >= filter_lower_bound) & (angle_data <= filter_upper_bound)
            filter_masks[angle_name] = final_mask
            
            # Store all necessary information for the plot
            plot_info[angle_name] = {
                'original_data': angle_data,
                'dominant_mean': dominant_mean,
                'lower_bound': filter_lower_bound,
                'upper_bound': filter_upper_bound,
                'skipped': False
            }

        # Combine all filter conditions (AND logic)
        combined_mask = np.ones(len(synchronized_data), dtype=bool)
        for angle_name, mask in filter_masks.items():
            combined_mask &= mask
        ...
        self.logger.info(f"  Original points: {len(synchronized_data)}")
        self.logger.info(f"  Filtered points: {len(filtered_data)}")
        self.logger.info(f"  Total points removed: {points_removed} ({removal_ratio:.1f}%)")
        ...
        self.generate_pose_stability_plot(plot_info, combined_mask)
        
        return filtered_data

    def generate_pose_stability_plot(self, plot_info, combined_mask):
        """
        Generates and saves a histogram plot visualizing the pose stability filtering.
        Shows retained vs. removed points for each angle.
        """
        self.logger.info("Generating pose stability filter analysis plot...")
        
        fig, axes = plt.subplots(3, 1, figsize=(12, 18))
        fig.suptitle('Pose Stability Filtering Analysis', fontsize=16, fontweight='bold')

        for i, (angle_name, info) in enumerate(plot_info.items()):
            ax = axes[i]
            original_data = info['original_data']

            # Separate data into retained and removed based on the FINAL combined mask
            data_retained = original_data[combined_mask]
            data_removed = original_data[~combined_mask]

            # Plot histograms
            # Bins need to be consistent for both histograms to stack correctly
            min_val, max_val = np.min(original_data), np.max(original_data)
            bins = np.linspace(min_val, max_val, 100)

            ax.hist(data_removed, bins=bins, color=COLOR_SCHEME['world_final_filtered'], alpha=0.7, label='Removed')
            ax.hist(data_retained, bins=bins, color=COLOR_SCHEME['residuals_differences'], alpha=0.8, label='Retained')
            
            ax.set_title(f'{angle_name.capitalize()} Angle Distribution')
            ax.set_xlabel('Angle (degrees)')
            ax.set_ylabel('Point Count')

            # Add annotations if filtering was not skipped for this axis
            if not info['skipped']:
                dominant_mean = info['dominant_mean']
                lower_bound = info['lower_bound']
                upper_bound = info['upper_bound']

                # Shade the retained area
                ax.axvspan(lower_bound, upper_bound, color=COLOR_SCHEME['residuals_differences'], alpha=0.15, label='Retained Region')

                # Add vertical lines for mean and boundaries
                ax.axvline(dominant_mean, color='black', linestyle='--', linewidth=2, label=f'Dominant Mean: {dominant_mean:.2f}°')
                ax.axvline(lower_bound, color=COLOR_SCHEME['tolerance_line'], linestyle=':', linewidth=2, label='Filter Boundary')
                ax.axvline(upper_bound, color=COLOR_SCHEME['tolerance_line'], linestyle=':', linewidth=2)
            else:
                ax.text(0.5, 0.5, 'Filtering Skipped for this Axis',
        ...
        # Save the figure
        plot_file = os.path.join(self.run_output_dir, "pose_stability_filter_analysis.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
        plt.close()
        
        self.logger.info(f"Pose stability analysis plot saved to: {plot_file}")
