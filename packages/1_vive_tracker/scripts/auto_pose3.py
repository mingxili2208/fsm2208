...
def get_timestamped_log_filename():
    ...
    return raw_filename, filtered_filename
        ...
        if recording:
            ...
            try:
                ...
                if parsed_data:
                    ...
                    print(f"[原始] A69: distance_2(X)={raw_distance_2:.3f}, distance_3(Z)={raw_distance_3:.3f}")
                    print(f"[滤波] A69: distance_2(X)={filtered_distance_2:.3f}, distance_3(Z)={filtered_distance_3:.3f}")
...
def save_data_to_file(filename, data, is_filtered=False):
    with open(filename, "w") as f:
        if is_filtered:
            f.write("Timestamp, Distance_2(X), Distance_3(Z), X, Y, Z, Roll, Yaw, Pitch, CalibrationMode, IsValid\n")
            ...
            f.write("Timestamp, Distance_2(X), Distance_3(Z), X, Y, Z, Roll, Yaw, Pitch, CalibrationMode\n")
...
def on_release(key):
    ...
    if key == keyboard.Key.esc:
        # 使用带时间戳的日志文件名
        raw_file, filtered_file = get_timestamped_log_filename()
        print(f"\n[系统] 正在保存数据...")
