...
class TrackerSystem:
    ...
    def serial_listener(self):
        ...
        while self.running:
            if self.recording and not self.single_record_mode:
    ...
    def on_press(self, key):
        ...
        try:
            if key.char == 'r' and not self.recording and not self.single_record_mode:
                self.recording = True
                logger.info("\n[System] Starting data recording...")
            elif key.char == 'e' and self.recording and not self.single_record_mode:
                self.recording = False
                logger.info("\n[System] Stopping data recording")
            elif key.char == 't' and not self.single_record_mode:
                self.single_record_mode = True
                logger.info("\n[System] Starting single record mode...")
                ...
                try:
                    ...
                    logger.info(f"[Data] Recorded Euler data: timestamp={timestamp:.3f}, "
        ...
        if key == keyboard.Key.esc:
            ...
            if self.euler_data:
                with open(EULER_CSV, "w") as euler_file:
                    ...
                    for timestamp, coord, angle in self.euler_data:
                        euler_entry = f"{timestamp:.3f},{coord[0]:.4f},{coord[1]:.4f},{coord[2]:.4f},{coord[3]:.4f},{coord[4]:.4f},{coord[5]:.4f},{angle:.2f}\n"
                        euler_file.write(euler_entry)
                logger.info(f"[System] Euler data saved to {EULER_CSV}")
    ...
    def calculate_transformations(self):
        ...
        try:
            ...
            # Define orientations based on Euler data or use default orientations
            try:
                euler_df = pd.read_csv(EULER_CSV)
                
                # Process orientation data
                # This is a simplified approach - in a real implementation, you would
                # need to map the tracker angles to the corresponding orientations
                orientations_A = []
                orientations_B = []
                ...
                logger.warning(f"Error loading Euler data: {e}")
                logger.warning("Using default orientation data")
                
                # Default orientation data
                orientations_A = [
                    [np.radians(0), np.radians(56.0226), np.radians(0)],
                    [np.radians(0), np.radians(56.6886), np.radians(0)],
                    [np.radians(0), np.radians(148.0405), np.radians(0)],
                    [np.radians(0), np.radians(-123.0951), np.radians(0)],
                    [np.radians(0), np.radians(-32.5727), np.radians(0)]
                ]
                
                orientations_B = [
                    [np.radians(0), np.radians(-90), np.radians(0)],
                    [np.radians(0), np.radians(-90), np.radians(0)],
                    [np.radians(0), np.radians(0), np.radians(0)],
                    [np.radians(0), np.radians(90), np.radians(0)],
                    [np.radians(0), np.radians(180), np.radians(0)]
                ]
...
def main():
    ...
    logger.info("Press 'R' to start continuous recording")
    logger.info("Press 'E' to stop continuous recording")
    logger.info("Press 'T' for single point recording with angle input")
