...
class TrackerSystem:
    ...
    def review_latest_euler_data(self):
        """Review and potentially modify the latest recorded Euler data"""
        verify = input("Do you want to verify or modify this record? (y/n): ").strip().lower()
        
        if verify == 'y':
            # Load the current euler.csv
            euler_df = pd.read_csv(EULER_CSV)
            
            # Display the last few records
            num_records = min(10, len(euler_df))
            logger.info(f"\nLast {num_records} records in Euler data:")
            for i in range(-num_records, 0):
                row = euler_df.iloc[i]
                logger.info(f"Index {len(euler_df) + i}: timestamp={row['Timestamp']:.3f}, "
                          f"x={row['X']:.4f}, y={row['Y']:.4f}, z={row['Z']:.4f}, "
                          f"roll={row['Roll']:.4f}, yaw={row['Yaw']:.4f}, pitch={row['Pitch']:.4f}, "
                          f"angle={row['DesiredAngle']:.2f}")
            
            # Ask if user wants to delete or modify any record
            action = input("Do you want to delete or modify a record? (d=delete/m=modify/n=none): ").strip().lower()
            
            if action == 'd':
                try:
                    idx = int(input("Enter the index of the record to delete: "))
                    if 0 <= idx < len(euler_df):
                        euler_df = euler_df.drop(idx).reset_index(drop=True)
                        euler_df.to_csv(EULER_CSV, index=False)
                        logger.info(f"Record at index {idx} deleted.")
                    else:
                        logger.error(f"Invalid index. Must be between 0 and {len(euler_df)-1}.")
                except ValueError:
                    logger.error("Invalid index. Please enter a numeric value.")
                    
            elif action == 'm':
                try:
                    idx = int(input("Enter the index of the record to modify: "))
                    if 0 <= idx < len(euler_df):
                        new_angle = float(input("Enter the new angle value: "))
                        euler_df.at[idx, 'DesiredAngle'] = new_angle
                        euler_df.to_csv(EULER_CSV, index=False)
                        logger.info(f"Angle for record at index {idx} modified to {new_angle}.")
                    else:
                        logger.error(f"Invalid index. Must be between 0 and {len(euler_df)-1}.")
                except ValueError:
        ...
        try:
            if key.char == 'r' and not self.recording:
                ...
                try:
                    ...
                    # Ask if user wants to verify/modify the recorded data
                    self.review_latest_euler_data()
    ...
    def on_release(self, key):
        ...
        if key == keyboard.Key.esc:
            ...
            # Ask for angle offset adjustments
            angle_neg = 122
            angle_pos = 55
            
            adjust_angles = input("Do you want to adjust the angle offsets? (y/n): ").strip().lower()
            if adjust_angles == 'y':
                try:
                    angle_neg = float(input(f"Enter the negative yaw offset (currently {angle_neg}): "))
                    angle_pos = float(input(f"Enter the positive yaw offset (currently {angle_pos}): "))
                    logger.info(f"Using new angle offsets: negative yaw: +{angle_neg}°, positive yaw: -{angle_pos}°")
                except ValueError:
            ...
            if os.path.exists(LASER_TRACKER_CSV):
                ...
                process_laser_tracker_data(LASER_TRACKER_CSV, CORRECTED_LASER_TRACKER_CSV, angle_neg, angle_pos)
